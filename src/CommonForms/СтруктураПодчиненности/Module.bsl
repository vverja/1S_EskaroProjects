...
Процедура ВывестиРодительскиеЭлементыДерева(СтрокиДерева,Макет,УровеньРекурсии)
	...
	Счётчик =  СтрокиДерева.Количество();
	Пока Счётчик >0 Цикл
		
		ТекущаяСтрокаДерева = СтрокиДерева.Получить(Счётчик -1);
...
Функция ПолучитьСписокДокументовПоКритериюОтбора(ЗначениеКритерияОтбора)
	...
	Для Каждого ЭлементСостава ИЗ Метаданные.КритерииОтбора.СвязанныеДокументы.Состав Цикл
		...
		Если ЭлементСостава = Неопределено
		 Или Не ЭлементСостава.Тип.СодержитТип(ТипЗначениеКритерияОтбора) Тогда
			Продолжить;
		КонецЕсли;

		СтруктураДанных = РазобратьПутьКОбъектуМетаданных(ЭлементСостава.ПолноеИмя());

		ЕСли Не ПравоДоступа("Чтение", СтруктураДанных.Метаданные) Тогда
			Продолжить;
		КонецЕсли;

		ИмяОбъекта       = СтруктураДанных.ТипОбъекта + "." + СтруктураДанных.ВидОбъекта;
		ТекущаяСтрокаГДЕ = "ГДЕ " + СтруктураДанных.ВидОбъекта + "." +СтруктураДанных.ИмяРеквизита
				+ " = &ЗначениеКритерияОтбора";

		ТекстЗапроса = ТекстЗапроса + ?(ТекстЗапроса = "", "ВЫБРАТЬ РАЗРЕШЕННЫЕ ", "ОБЪЕДИНИТЬ ВСЕ ВЫБРАТЬ ") + "
		|" + СтруктураДанных.ВидОбъекта +".Ссылка
		|ИЗ " + ИмяОбъекта + "." + СтруктураДанных.ИмяТаблЧасти + " КАК " + СтруктураДанных.ВидОбъекта + "
...
Процедура Обновить(Команда)
	...
	ДокументСсылка = ИсходныйДокумент;
