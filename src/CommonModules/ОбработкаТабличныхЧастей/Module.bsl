...
Процедура ПриИзмененииЕдиницыТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	ИмяТабличнойЧасти      = ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти);
	МетаданныеДокумента    = ДокументОбъект.Метаданные();
	СтруктураРеквизитовЦен = Новый Структура("Цена, ЦенаВРознице, ЦенаПередачи, ЦенаНоминальная");
...
Процедура ПроверкаМинимальнойЦены(ДатаСкидок, ТаблицаТоваров, МинимальныеЦены,
	...
	Для Каждого СтрокаТовара ИЗ ТаблицаТоваров Цикл
		...
		Если УчитыватьНДС Тогда // рассчитаем сумму НДС строки таблицы значений
			СтрокаТовара.СуммаНДС = Ценообразование.РассчитатьСуммуНДС(СтрокаТовара.Сумма, УчитыватьНДС, СуммаВключаетНДС,
			   Ценообразование.ПолучитьСтавкуНДС(СтрокаТовара.СтавкаНДС));
		КонецЕсли;
	КонецЦикла;
...
Функция ЗапросПоСкидкам(ДатаСкидок, МассивНоменклатуры, МассивХарактеристик, МассивКачество,
	...
	|	РегСкидкиНоменклатуры.ДокВыбТоваров
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|		ВЫБОР
	|			КОГДА РегСкидкиНоменклатуры.Условие = ЗНАЧЕНИЕ(Перечисление.УсловияСкидкиНаценки.ПоСуммеТовараДокумента) ИЛИ РегСкидкиНоменклатуры.Условие = ЗНАЧЕНИЕ(Перечисление.УсловияСкидкиНаценки.ПоСуммеТовараДокумента2)
	|				ТОГДА РегСкидкиНоменклатуры.Регистратор
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ КАК ДокВыбТоваров
	|	ИЗ
	...
	|				ТОГДА РегСкидкиНоменклатуры.Регистратор
	|			ИНАЧЕ НЕОПРЕДЕЛЕНО
	|		КОНЕЦ) КАК РегСкидкиНоменклатуры
	|ПО
...
Процедура РассчитатьСкидкиПриПродаже(ДокументОбъект, ТабличнаяЧастьНоменклатуры,
	...
	//проверим на прохождение одного товара по нескольким видам скидки
	ТаблПроверкиДублей = ТаблицаСкидок.Скопировать();
	ТаблПроверкиДублей.Колонки.Добавить("Колво");
	ТаблПроверкиДублей.ЗаполнитьЗначения(1, "Колво");
	ТаблПроверкиДублей.Свернуть("Номенклатура, ХарактеристикаНоменклатуры, Качество", "Колво");
	Для Каждого СтрокаДублей ИЗ ТаблПроверкиДублей Цикл
		Если СтрокаДублей.Колво > 1 Тогда
			Сообщить("Номенклатура - " + СтрокаДублей.Номенклатура + " учавствует в:");
			МСДублей = ТаблицаСкидок.НайтиСтроки(Новый Структура("Номенклатура, ХарактеристикаНоменклатуры, Качество", СтрокаДублей.Номенклатура, СтрокаДублей.ХарактеристикаНоменклатуры, СтрокаДублей.Качество));
			Для Каждого ЭлемДублей ИЗ МСДублей Цикл
				Сообщить("		" + ЭлемДублей.НУсловие + СокрЛП(ЭлемДублей.ДокВыбТоваров));
				//ТаблицаСкидок.Удалить(ЭлемДублей);
			КонецЦикла;
		КонецЕсли;	
	КонецЦикла;
	
	//обработаем вариант скидки по превышению суммы выбранных товаров /// smts
	МС = ТаблицаСкидок.НайтиСтроки(Новый Структура("НУсловие", Перечисления.УсловияСкидкиНаценки.ПоСуммеТовараДокумента));
	ТЗВыбТоваров = ТаблицаСкидок.Скопировать(МС,);
	
	ТЗ_ГруппировкиДок = ТЗВыбТоваров.Скопировать();
	ТЗ_ГруппировкиДок.Свернуть("ДокВыбТоваров, НЗначениеУсловия");
	
	Для Каждого СтрокаДок ИЗ ТЗ_ГруппировкиДок Цикл
		
		СуммаПроверки = 0;
		Для Каждого СтрокаТЧДок ИЗ СтрокаДок.ДокВыбТоваров.Товары Цикл
			
			МС_Найденных = ТаблицаТоваров.НайтиСтроки(Новый Структура("Номенклатура", СтрокаТЧДок.Номенклатура)); //могут быть несколько строк одной номенклатуры
			ТЗ_Сумм = ТаблицаТоваров.Скопировать(МС_Найденных);
			СуммаПроверки = СуммаПроверки + ТЗ_Сумм.Итог("СуммаБезСкидки");
			//Если СтрокаТЧДок.Номенклатура = ТаблицаТоваров.Номенклатура Тогда
			//	СуммаПроверки = СуммаПроверки + ТаблицаТоваров.СуммаБезСкидки;
			//КонецЕсли;
		КонецЦикла;
		
		//определим выполняется ли условие на ограничение суммы выбранных товаров
		Если СтрокаДок.НЗначениеУсловия > СуммаПроверки Тогда //условие не выполняется - удаляем такие строки из таблицы скидок
			МС_Удалить = ТаблицаСкидок.НайтиСтроки(Новый Структура("ДокВыбТоваров", СтрокаДок.ДокВыбТоваров));
			Для Каждого ЭлементМС ИЗ МС_Удалить Цикл
				ТаблицаСкидок.Удалить(ЭлементМС);
			КонецЦикла;
		КонецЕсли;
	...
	// 2 //обработаем вариант скидки по превышению суммы выбранных товаров /// smts
	МС = ТаблицаСкидок.НайтиСтроки(Новый Структура("НУсловие", Перечисления.УсловияСкидкиНаценки.ПоСуммеТовараДокумента2));
	ТЗВыбТоваров = ТаблицаСкидок.Скопировать(МС,);
	...
	КонецЦикла;
	// проверили граничные условия - если граничные условия не соблюдены - удалили строки скидок, оставшиеся обработаем дальше
	МС = ТаблицаСкидок.НайтиСтроки(Новый Структура("НУсловие", Перечисления.УсловияСкидкиНаценки.ПоСуммеТовараДокумента2));
	...
	Если МС.Количество() > 0 Тогда
		Для Каждого СтрокаТовары ИЗ ТаблицаТоваров Цикл
			...
			Для каждого ЭлементМС Из МС Цикл
				Если ЭлементМС.Номенклатура = СтрокаТовары.Номенклатура Тогда
					НеДобавлять = Истина;
					Прервать;
				КонецЕсли;
			КонецЦикла; 
			...
			НССкидок = ТаблицаСкидок.Добавить();
			ЗаполнитьЗначенияСвойств(НССкидок, МС[0]);
			НССкидок.Номенклатура = СтрокаТовары.Номенклатура;
			НССкидок.ХарактеристикаНоменклатуры = СтрокаТовары.ХарактеристикаНоменклатуры;
			НССкидок.Качество = СтрокаТовары.Качество;
			НССкидок.НПроцентСкидкиНаценки = ?(ЭлементМС.НПроцентСкидкиНаценки = 0, ЭлементМС.ДокВыбТоваров.ПроцентСкидкиНаценки, ЭлементМС.НПроцентСкидкиНаценки);
			//НССкидок.ДокВыбТоваров = ЭлементМС.ДокВыбТоваров;
			//НССкидок.НУсловие = ЭлементМС.НУсловие;
			//НССкидок.НЗначениеУсловия = ЭлементМС.НЗначениеУсловия;
			//НССкидок.НОграничениеСкидкиНаценки = ЭлементМС.НОграничениеСкидкиНаценки;
			//НССкидок.ЦОграничениеСкидкиНаценки = ЭлементМС.ЦОграничениеСкидкиНаценки;
		КонецЦикла;
	КонецЕсли; 
					 ...
					 Или СтрокаСкидок.ЦУсловие <> УсловиеКолво) Тогда
						Если СтрокаСкидок.НПроцентСкидкиНаценки <> Null Тогда
							...
							Если МаксСуммаСкидки = Неопределено Или МаксСуммаСкидки < СуммаСкидки Тогда
								...
								ПроцентСкидки = Окр((СуммаСкидки / СтрокаТЧ.СуммаБезСкидки) * 100, 4);
...
Процедура ЗаполнитьСтавкуНДСТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВидДокумента = "") Экспорт
	...
	ОписаниеТиповТабличнаяЧастьОбработки = Новый ОписаниеТипов("ОбработкаТабличнаяЧастьСтрока.ОбработкаТабличнойЧастиТовары.Товары, ОбработкаТабличнаяЧастьСтрока.ФормированиеНалоговыхНакладных.Товары");
	...
	Если РеквизитыТабличнойЧасти.Найти("СтавкаНДС")<>неопределено Тогда
		...
		Если НеЯвляетсяРезидентом И ВидДокумента = "Приобретение" Тогда	
...
Процедура ЗаполнитьЦенуПродажиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	Если ЗначениеЗаполнено(Цена) Тогда
		СтрокаТабличнойЧасти.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
					...
					Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС),0));
	КонецЕсли;
...
Процедура ЗаполнитьЕдиницуЦенуПродажиТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета,
	...
	Если ТекСклад.ВидСклада = Перечисления.ВидыСкладов.Розничный Тогда
		...
		КонецЕсли;
	Иначе
		Если НЕ ЗначениеЗаполнено(ТекТипЦен) Тогда
			СтрокаТабличнойЧасти.ЕдиницаИзмерения = СтрокаТабличнойЧасти.Номенклатура.ЕдиницаХраненияОстатков;
			СтрокаТабличнойЧасти.Цена = 0;
		Иначе
			Цена = Ценообразование.ПолучитьЦенуНоменклатуры(СтрокаТабличнойЧасти.Номенклатура, ХарактеристикаНоменклатуры,
	...
	Иначе
		...
		Иначе
			СтрокаТабличнойЧасти.Цена = Ценообразование.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
			                            ...
			                            ?(ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СтавкаНДС", МетаданныеДокумента, ИмяТабличнойЧасти), Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС),0));
		КонецЕсли;
...
Процедура РассчитатьРозничнуюЦенуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	Если МетаданныеДокумента.Реквизиты.Найти("СуммаВключаетНДС") <> Неопределено Тогда
		...
		СтавкаНДС        = Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС);
	Иначе
		СуммаВключаетНДС = Истина;
		СтавкаНДС        = Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.Номенклатура.СтавкаНДС);
	КонецЕсли;
...
Процедура РассчитатьСуммуТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект, СпособРасчета = Неопределено) Экспорт
	...
	Если ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.ЗаказПокупателя") ИЛИ ТипЗнч(ДокументОбъект) = Тип("ДокументОбъект.РеализацияТоваровУслуг") Тогда
		СтрокаТабличнойЧасти.Сумма = Окр(Сумма - СуммаСкидки, 2, РежимОкругления.Окр15как20);
	Иначе 
		СтрокаТабличнойЧасти.Сумма = Сумма - СуммаСкидки;
	КонецЕсли;
	
	//Смартис Лиманчук начало 04.10.2012
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СуммаНормативнойСебестоимости", МетаданныеДокумента,
		                                 ИмяТабличнойЧасти) Тогда
		СтрокаТабличнойЧасти.СуммаНормативнойСебестоимости = СтрокаТабличнойЧасти.Сумма;
	КонецЕсли;
	//Смартис Лиманчук окончание 04.10.2012
	
	//Смартис Лиманчук начало 13.05.2013
	//в документе Инвентаризация, для устранения ошибок округления, 
	//если количество после изменения вручную оказывается равно учетному количеству,
	//сумму устанавливаем равной учетной сумме
	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("КоличествоУчет", МетаданныеДокумента,
		                                 ИмяТабличнойЧасти) Тогда
		Если СтрокаТабличнойЧасти.КоличествоУчет = СтрокаТабличнойЧасти.Количество Тогда
		    СтрокаТабличнойЧасти.Сумма = СтрокаТабличнойЧасти.СуммаУчет;
		КонецЕсли;
	КонецЕсли;
	//Смартис Лиманчук окончание 13.05.2013
	...
	Если (ДокументОбъект.УчитыватьНДС) 
	   И (НЕ ДокументОбъект.СуммаВключаетНДС) Тогда
		...
		Если СуммаПлановойСебестоимости <> 0 Тогда
			...
			СуммаНДСПлановойСебестоимости = Ценообразование.РассчитатьСуммуНДС(СуммаПлановойСебестоимости,
			                                ДокументОбъект.УчитыватьНДС,
			                                ДокументОбъект.СуммаВключаетНДС,
			                                Ценообразование.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС));
...
Процедура РассчитатьПлановуюСебестоимостьВСтрокахТабЧасти(ДокументОбъект, ВалютаРегламентированногоУчета) Экспорт
	...
	Для каждого СтрокаТабличнойЧасти ИЗ ДокументОбъект.Товары Цикл
		// Плановая себестоимость не расчитываеться для БСУ.
		Если ТипЗнч(СтрокаТабличнойЧасти.Номенклатура) = Тип("СправочникСсылка.Номенклатура") 
		   И НЕ СтрокаТабличнойЧасти.Номенклатура.Пустая()
		   И СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры.БланкСтрогогоУчета Тогда			
			Продолжить;
		КонецЕсли; // ЕстьНоменклатураВТЧ 
...
Процедура ЗаполнитьПлановуюСебестоимостьНаОсновании(ДокументОбъект, ДокументОснование, ВалютаРегламентированногоУчета) Экспорт
	...
	Запрос.Текст =
		...
		|	ГДЕ
		|		Док.Ссылка = &Сделка
		|		И НЕ Док.Номенклатура.ВидНоменклатуры.БланкСтрогогоУчета";
...
Процедура ПересчитатьНДСсУчетомПогрешностиОкругления(ТабличнаяЧасть, ДокументОбъект, ПогрешностиОкругления, ПредставлениеТабличнойЧасти = "", ИмяРеквизитаСумма    = "Сумма", ИмяРеквизитаСуммаНДС = "СуммаНДС", ДополнениеКТексту ="") Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТабличнаяЧасть Цикл
		...
		СтрокаТаблицы[ИмяРеквизитаСуммаНДС] = Ценообразование.РассчитатьСуммуНДСсУчетомПогрешности(СтрокаТаблицы[ИмяРеквизитаСумма],
	                                                   ДокументОбъект.УчитыватьНДС,
	                                                   ДокументОбъект.СуммаВключаетНДС,
	...
	#Если Клиент Тогда
		...
		Если СуммаНДСПослеОкругления <> СуммаНДСДоОкругления Тогда
			...
			ОбщегоНазначения.СообщитьОбОшибке("После исправления погрешностей округления сумма НДС"+ДополнениеКТексту+" табличной части " + ПредставлениеТабличнойЧасти + " изменилась (было "
			+ ОбщегоНазначения.ФорматСумм(СуммаНДСДоОкругления, ДокументОбъект.ВалютаДокумента) + ", стало " + ОбщегоНазначения.ФорматСумм(СуммаНДСПослеОкругления, ДокументОбъект.ВалютаДокумента) + ")");
			
			// изменилась общая сумма документа - сообщим об этом пользователю 
			Если (ДокументОбъект.УчитыватьНДС)
				И (Не ДокументОбъект.СуммаВключаетНДС) Тогда
				ОбщегоНазначения.СообщитьОбОшибке("После исправления погрешностей округления общая сумма"+ДополнениеКТексту+" табличной части " + ПредставлениеТабличнойЧасти + " изменилась (было "
				+ ОбщегоНазначения.ФорматСумм(СуммаДоОкругления, ДокументОбъект.ВалютаДокумента) + ", стало " + ОбщегоНазначения.ФорматСумм(СуммаПослеОкругления, ДокументОбъект.ВалютаДокумента) + ")");
			КонецЕсли;	
		КонецЕсли;
	#КонецЕсли	
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ОБРАБОТКИ БСУ 

Процедура БСУРассчитатьНоминальныеРеквизитыТЧ(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт
	...
	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
		...
		Если НЕ СтрокаТабличнойЧасти.Номенклатура.ВидНоменклатуры.УчитываетсяПоНоминальнойСтоимости Тогда
			
            Если МетаданныеДокумента.Реквизиты.Найти("СуммаВключаетНДС") <> Неопределено Тогда
				СуммаВключенногоНДС = ?(ДокументОбъект.СуммаВключаетНДС, СтрокаТабличнойЧасти.СуммаНДС, 0);
			Иначе
				СуммаВключенногоНДС = 0;
			КонецЕсли;
			
			СтрокаТабличнойЧасти.СуммаНоминальная = СтрокаТабличнойЧасти.Сумма - СуммаВключенногоНДС;
			СтрокаТабличнойЧасти.ЦенаНоминальная  = СтрокаТабличнойЧасти.Цена  - ?(СтрокаТабличнойЧасти.Количество, СуммаВключенногоНДС/СтрокаТабличнойЧасти.Количество, СуммаВключенногоНДС);
			
		Иначе  //заполнение для бланков, которые НЕ учитываются по номинальной стоимости	
...
//
Функция БСУПолучитьЦенуНоминальную(Номенклатура, Дата) Экспорт 

	ТЗНоминальныхЦен = РегистрыСведений.НоминальнаяСтоимостьБланковСтрогогоУчета.СрезПоследних(Дата, Новый Структура("Номенклатура", Номенклатура));			   
	
	Возврат ?(ТЗНоминальныхЦен.Количество(), ТЗНоминальныхЦен[0].Цена, 0);

КонецФункции // БСУПолучитьЦенуНоминальную

Процедура ПриИзмененииПродукцииТабЧасти(СтрокаТабличнойЧасти, ДокументОбъект) Экспорт

	ИмяТабличнойЧасти = ОбщегоНазначения.ПолучитьИмяТабличнойЧастиПоСсылкеНаСтроку(СтрокаТабличнойЧасти);

	МетаданныеДокумента = ДокументОбъект.Метаданные();

	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("СерияПродукции", МетаданныеДокумента, ИмяТабличнойЧасти)
	   И СтрокаТабличнойЧасти.Продукция <> СтрокаТабличнойЧасти.СерияПродукции.Владелец Тогда
		СтрокаТабличнойЧасти.СерияПродукции = Неопределено;
	КонецЕсли;

	Если ОбщегоНазначения.ЕстьРеквизитТабЧастиДокумента("ХарактеристикаПродукции", МетаданныеДокумента, ИмяТабличнойЧасти)
	   И СтрокаТабличнойЧасти.Продукция <> СтрокаТабличнойЧасти.ХарактеристикаПродукции.Владелец Тогда
		СтрокаТабличнойЧасти.ХарактеристикаПродукции = Неопределено;
	КонецЕсли;

КонецПроцедуры // ПриИзмененииПродукцииТабЧасти()



// Проверяет наличие хотя бы одного заполненного реквита в указанной колонке табличной части
//
Функция ЕстьЗаполненныйРеквизитВТабличнойЧасти(ТабличнаяЧасть, ИмяКолонки, ПустоеЗначение)

	КоличествоСтрок 		= ТабличнаяЧасть.Количество();
	КоличествоНепустыхСтрок = ТабличнаяЧасть.НайтиСтроки(Новый Структура(ИмяКолонки, ПустоеЗначение)).Количество();
		
	Возврат КоличествоСтрок > 0 И НЕ (КоличествоНепустыхСтрок = КоличествоСтрок);

КонецФункции // 
 
// Проверяет наличие хотя бы одного заполненного Счета учета НДС в табличной части
//
Функция ЕстьЗаполненныйСчетУчетаНДСВТабличнойЧасти(ТабличнаяЧасть, ИмяКолонки = "СчетУчетаНДС", ПустоеЗначение = Неопределено) Экспорт
	
	Если ПустоеЗначение = Неопределено Тогда
		ПустоеЗначение = ПланыСчетов.Хозрасчетный.ПустаяСсылка();
	КонецЕсли;
	
	Возврат ЕстьЗаполненныйРеквизитВТабличнойЧасти(ТабличнаяЧасть, ИмяКолонки, ПустоеЗначение);
