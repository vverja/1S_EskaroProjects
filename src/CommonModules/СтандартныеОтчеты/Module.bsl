...
Функция ВыбратьСчет(Тип = "Группировка", НачальноеЗначениеВыбора = Неопределено) Экспорт
	...
	ФормаВыбораСчета.ПланСчетовСписок.Порядок.Установить("Код Возр");
...
Функция ПолучитьСтруктуруПоказателейОтчета(БУ = Истина, НУ = Истина, Контроль = Истина, ВалютнаяСумма = Ложь, Количество = Ложь, РазвернутоеСальдо = Ложь) Экспорт
	
	Возврат Новый Структура("БУ, НУ, Контроль, ВалютнаяСумма, Количество, РазвернутоеСальдо", 
							Новый Структура("Значение, Использование, Представление", Истина, БУ,                НСтр("ru='БУ (данные бухгалтерского учета)';uk='БО (дані бухгалтерського обліку)'")),
							Новый Структура("Значение, Использование, Представление", Ложь,   НУ,                НСтр("ru='НУ (данные налогового учета)';uk='ПО (дані податкового обліку)'")),
							Новый Структура("Значение, Использование, Представление", Ложь,   Контроль,          НСтр("ru='Разница (БУ - НУ)';uk='Різниця (БО - ПО)'")),
							Новый Структура("Значение, Использование, Представление", Ложь,   ВалютнаяСумма,     НСтр("ru='Валютная сумма';uk='Валютна сума'")),
							Новый Структура("Значение, Использование, Представление", Ложь,   Количество,        НСтр("ru='Количество';uk='Кількість'")),
							Новый Структура("Значение, Использование, Представление", Ложь,   РазвернутоеСальдо, НСтр("ru='Развернутое сальдо';uk='Розгорнуте сальдо'")));
...
Функция ПолучитьТекстЗаголовка(ОтчетОбъект, ЗаголовокТекст = "", ОрганизацияВНачале = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ОтчетОбъект.Организация) Тогда
		СведенияОбОрганизации    = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(ОтчетОбъект.Организация,,,,КодЯзыкаПечать);
		ТекстОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,",,КодЯзыкаПечать);
...
Функция ПолучитьПредставлениеПериодичности(Периодичность) 
	...
	Если Периодичность = 6 Тогда
		Представление = НСтр("ru='по дням;uk='по дням'");
	ИначеЕсли Периодичность = 7 Тогда
		Представление = НСтр("ru='по неделям';uk='по тижнях'");
	ИначеЕсли Периодичность = 8 Тогда
		Представление = НСтр("ru='по декадам';uk='по декадах'");
	ИначеЕсли Периодичность = 9 Тогда
		Представление = НСтр("ru='по месяцам';uk='по місяцях'");
	ИначеЕсли Периодичность = 10 Тогда
		Представление = НСтр("ru='по кварталам';uk='по кварталах'");
	ИначеЕсли Периодичность = 11 Тогда
		Представление = НСтр("ru='по полугодиям';uk='по півріччях'");
	ИначеЕсли Периодичность = 12 Тогда
		Представление = НСтр("ru='по годам';uk='по роках'");
	КонецЕсли;
...
Функция ПолучитьТекстОбособленныхПодразделений(Организация) Экспорт
	...
	ТекстОрганизации = ТекстОрганизации + НСтр("ru=' с обособленными подразделениями';uk=' с відособленими підрозділами'");
...
Функция ПолучитьТекстОрганизация(ОтчетОбъект = Неопределено, Организация = Неопределено, ВключатьОбособленныеПодразделения = Ложь) Экспорт
	...
	Если ЗначениеЗаполнено(Организация) Тогда
		...
		КонецПопытки; 
		СведенияОбОрганизации = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Организация, НачалоПериода);
		ТекстОрганизации = ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОбОрганизации, "НаименованиеДляПечатныхФорм,");
		Если ПустаяСтрока(ТекстОрганизации) Тогда
			ТекстОрганизации = СведенияОбОрганизации.Представление;
		КонецЕсли;
...
Процедура ОтборНачалоВыбора(ОтчетОбъект, ФормаОтчета, Элемент, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли ТипЗнч(Элемент.Значение) = Тип("СправочникСсылка.ЗначенияСвойствОбъектов") Тогда
		...
		ИмяХарактеристики = СтрЗаменить(Сред(Поле, ПозицияНазванияХарактеристики + 1), НСтр("ru=' (доп. реквизит)';uk=' (дод. реквізит)'"), "");
...
Процедура ПоказателиОтчетаПриИзмененииФлажка(ОтчетОбъект, Элемент, Колонка) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ЗначениеПоказателей Цикл
		Если СтрокаТаблицы.Показатель <> НСтр("ru='Развернутое сальдо';uk='Розгорнуте сальдо'") Тогда
...
Процедура УправлениеОтображениемПанелиДополнительныеНастройки(ФормаОтчета, Видимость = Истина) Экспорт
	...
	Если Видимость Тогда
		ФормаОтчета.ЭлементыФормы.НадписьДополнительныеНастройки.Заголовок = НСтр("ru='Скрыть дополнительные настройки';uk='Сховати додаткові настройки'");
	...
	Иначе
		ФормаОтчета.ЭлементыФормы.НадписьДополнительныеНастройки.Заголовок = НСтр("ru='Показать дополнительные настройки';uk='Показати додаткові настройки'");
...
Процедура УсловноеОформлениеПриВыводеСтроки(ОтчетОбъект, Элемент, ОформлениеСтроки, ДанныеСтроки) Экспорт 
	...
	Если ДанныеСтроки.Поля.Элементы.Количество() = 0 Тогда
		...
		ОформлениеСтроки.Ячейки.ОбластиДляКраткогоОтображенияЭлемента.Текст = НСтр("ru='<Все поля>';uk='<Усі поля>'");
		ОформлениеСтроки.Ячейки.ОбластиДляПодробногоОтображенияЭлемента.Текст = НСтр("ru='<Все поля>';uk='<Усі поля>'");
...
Процедура ЗаполнитьДополнительныеПоляПоУмолчанию(ОтчетОбъект) Экспорт
	...
	Для Каждого Группировка Из ДанныеОтчета.Группировка Цикл 
		...
		ИначеЕсли ТипПоля.СодержитТип(Тип("СправочникСсылка.РасходыБудущихПериодов")) Тогда
			...
			НоваяСтрока.Представление = СтандартныеОтчеты.ПолучитьСвойствоПоля(КомпоновщикНастроек, НоваяСтрока.Поле);
		ИначеЕсли ТипПоля.СодержитТип(Тип("ДокументСсылка.ПоступлениеТоваровУслуг"))
...
Процедура СохранитьКопиюРезультатаОтчета(ОтчетОбъект, ФормаОтчета, ПолноеИмяФайла = "") Экспорт
	...
	Диалог.Заголовок = НСтр("ru='Сохранить как';uk='Зберегти як'");
	...
	Диалог.Фильтр = НСтр("ru='Табличный документ (*.mxl)|*.mxl|';uk='Табличний документ (*.mxl)|*.mxl|'")
	      		  + НСтр("ru='Документ Microsoft Excel (*.xls)|*.xls|';uk='Документ Mіcrosoft Excel (*.xls)|*.xls|'");
	Если Диалог.Выбрать() Тогда
		...
		Исключение
			Предупреждение(НСтр("ru='Ошибка при записи. Файл не записан.';uk='Помилка при записі. Файл не записаний.'"));
		КонецПопытки;		
	КонецЕсли;
...
Процедура ВывестиОтчет(ОтчетОбъект, Результат, ДанныеРасшифровки, ВыводВФормуОтчета, ВнешниеНаборыДанных, ЗаполнятьДанныеРасшифровки = Истина, НастройкаКомпоновкиДанных = Неопределено) Экспорт
	...
	Исключение
		Вопрос(НСтр("ru='Отчет не сформирован!';uk='Звіт не сформований!'") + Символы.ПС + ТиповыеОтчеты.ПолучитьОписаниеРодительскойПричиныИнформацииОбОшибке(ИнформацияОбОшибке()), РежимДиалогаВопрос.ОК);
	КонецПопытки;
...
Функция ПроверитьЗаполнениеОбязательныхРеквизитов(ФормаОтчета) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомостьПоСчету"
		...
		Если Не ЗначениеЗаполнено(ФормаОтчета.Счет) Тогда
			Ответ = Вопрос(НСтр("ru='Для формирования отчета необходимо указать счет.';uk='Для формування звіту необхідно вказати рахунок.'") + Символы.ПС + НСтр("ru='Открыть план счетов?';uk='Відкрити план рахунків?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
			Если Ответ = КодВозвратаДиалога.Да Тогда
				Если ВвестиЗначение(ФормаОтчета.Счет, НСтр("ru='Выберите счет';uk='Виберіть рахунок'")) Тогда
					ЗаполнениеКорректно = Истина;
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотыМеждуСубконто" Тогда
		...
		Если КоличествоСубконто = 0 Тогда
			Ответ = Вопрос(НСтр("ru='Для формирования отчета необходимо указать вид субконто.';uk='Для формування звіту необхідно вказати вид субконто.'") + Символы.ПС + НСтр("ru='Открыть список видов субконто?';uk='Відкрити список видів субконто?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ФормаОтчета.СписокВидовСубконто.Добавить();
				Если ВвестиЗначение(ФормаОтчета.СписокВидовСубконто[0].Значение, НСтр("ru='Выберите вид субконто';uk='Виберіть вид субконто'"), Тип("ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные")) Тогда
					ЗаполнениеКорректно = Истина;
		...
		Если КоличествоКорСубконто = 0 Тогда
			Ответ = Вопрос(НСтр("ru='Для формирования отчета необходимо указать вид кор. субконто.';uk='Для формування звіту необхідно вказати вид кор. субконто.'") + Символы.ПС + НСтр("ru='Открыть список видов субконто?';uk='Відкрити список видів субконто?'"), РежимДиалогаВопрос.ДаНет, , КодВозвратаДиалога.Да);
			Если Ответ = КодВозвратаДиалога.Да Тогда
				ФормаОтчета.СписокВидовКорСубконто.Добавить();
				Если ВвестиЗначение(ФормаОтчета.СписокВидовКорСубконто[0].Значение, НСтр("ru='Выберите вид субконто';uk='Виберіть вид субконто'"), Тип("ПланВидовХарактеристикСсылка.ВидыСубконтоХозрасчетные")) Тогда
					ЗаполнениеКорректно = Истина;
					ФормаОтчета.ОбработкаИзмененияСоставаСубконто();
					ФормаОтчета.ОбновитьПанельНастроек();
				Иначе
...
Процедура ОбработкаРасшифровкиСтандартногоОтчета(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Иначе
		...
		ПредставлениеДляОСВПоСчету            = НСтр("ru='ОСВ по счету ';uk='ОСВ по рахунку '")   + Счет;
		ПредставлениеДляКарточкиСчета         = НСтр("ru='Карточка счета ';uk='Картка рахунку '") + Счет;
		ПредставлениеДляАнализаСчета          = НСтр("ru='Анализ счета ';uk='Аналіз рахунку '")   + Счет;
		ПредставлениеДляОборотыСчетаПоМесяцам = НСтр("ru='Обороты счета ';uk='Обороти рахунку '")  + Счет + НСтр("ru=' по месяцам';uk=' по місяцях'");
		ПредставлениеДляОборотыСчетаПоДням    = НСтр("ru='Обороты счета ';uk='Обороти рахунку '")  + Счет + НСтр("ru=' по дням';uk=' по днях'");
		...
		Если ПервыйЭлемент <> Неопределено Тогда
			Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомость" Тогда
				...
				Если Не ЕстьПоказатель И ЗначениеЗаполнено(ПервыйЭлемент.Значение)  И Не ПростойТип(ПервыйЭлемент.Значение) Тогда
					СписокПунктовМеню.Добавить(НСтр("ru='Открыть ""';uk='Відкрити ""'") + ПервыйЭлемент.Значение + """");
				КонецЕсли;
...
Процедура ОбработкаРасшифровкиОтчетовДляРуководителя(ОтчетОбъект, ФормаОтчета, Расшифровка, СтандартнаяОбработка) Экспорт
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ДоходыРасходы" Тогда
		Если Субконто1 <> НСтр("ru='Расходы';uk='Витрати'") И Субконто1 <> НСтр("ru='Продажи без НДС';uk='Продажі без ПДВ'") И Субконто1 <> НСтр("ru='Прочие доходы без НДС';uk='Інші доходи без ПДВ'")Тогда 
			Возврат;
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОстаткиДенежныхСредств" Тогда
		Если КорСубконто1 = Неопределено Тогда
			КорСубконто1 = НСтр("ru='Итого, грн';uk='Всього, грн'");
		КонецЕсли;
	...
	Если Период <> Неопределено Тогда
		...
		КонецЕсли;
	КонецЕсли;
	ТиповыеОтчеты.УстановитьПараметр(ФормаОтчета.КомпоновщикНастроекПромежуточнаяРасшифровка, "ВидОтчета", НСтр("ru='Отчет ';uk='Звіт '") + """" + ОтчетОбъект.Метаданные().Синоним + """");
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ЗадолженностьПокупателей"
		...
		Если Субконто1 <> Неопределено И Субконто2 <> Неопределено Тогда
			Показатель = НСтр("ru='Контрагент: ';uk='Контрагент:'") + Строка(Субконто1) + Символы.ПС + НСтр("ru='Договор: ';uk='Договір: '") + Строка(Субконто2);
		ИначеЕсли Субконто1 <> Неопределено Тогда
			Показатель = НСтр("ru='Контрагент: ';uk='Контрагент:'") + Строка(Субконто1);
		Иначе
			Показатель = НСтр("ru='По контрагентам';uk='По контрагентах'");
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОборотныеСредства" Тогда
		Если Субконто2 <> Неопределено Тогда
			Показатель = НСтр("ru='Показатель: ';uk='Показник: '") + Строка(Субконто2);
		Иначе
			Показатель = НСтр("ru='Показатель: ';uk='Показник: '") + Строка(Субконто1);
		КонецЕсли;
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.Продажи" Тогда
		Если ОтчетОбъект.РежимФормирования = 0 Тогда
			Если Субконто1 = Неопределено Тогда		
				Показатель = НСтр("ru='По номенклатурным группам';uk='По номенклатурних групах'");
			Иначе
				Показатель = НСтр("ru='Номенклатурная группа: ';uk='Номенклатурна група: '") + Строка(Субконто1);
			КонецЕсли;
		Иначе
			Если КорСубконто1 = Неопределено Тогда		
				Показатель = НСтр("ru='По контрагентам';uk='По контрагентах'");
			Иначе
				Показатель = НСтр("ru='Контрагент: ';uk='Контрагент:'") + Строка(КорСубконто1);
			КонецЕсли;
		КонецЕсли;
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОстаткиДенежныхСредств" Тогда
		Если Субконто1 <> Неопределено И Субконто2 <> Неопределено Тогда
			Показатель = "Вид: " + Строка(Субконто1) + Символы.ПС + НСтр("ru='Аналитика: ';uk='Аналітика: '") + Строка(Субконто2);
		ИначеЕсли Субконто1 <> Неопределено Тогда
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОборотныеСредства"
		...
		Если ВидРесурса = "начало" Тогда
			Показатель = Показатель + "На " + Формат(ОтчетОбъект.НачалоПериода, "ДФ=dd.MM.yyyy") + НСтр("ru=' (начало дня)';uk=' (початок дня)'");
		ИначеЕсли ВидРесурса = "конец" Тогда
			Показатель = Показатель + "На " + Формат(ОтчетОбъект.КонецПериода, "ДФ=dd.MM.yyyy") + НСтр("ru=' (конец дня)';uk=' (кінець дня)'");
		...
		ТиповыеОтчеты.УстановитьПараметр(ФормаОтчета.КомпоновщикНастроекПромежуточнаяРасшифровка, "КонецПериода" , ОтчетОбъект.КонецПериода);
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ОстаткиДенежныхСредств" Тогда
		Показатель = Показатель + "На " + Формат(ОтчетОбъект.Период, "ДФ=dd.MM.yyyy") + НСтр("ru=' (конец дня)';uk=' (кінець дня)'");
	...
	ИначеЕсли ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПокупателей" 
		...
		Показатель = Показатель + "На " + Формат(?(КонецПериода(Период, ОтчетОбъект.Интервал) > КонецДня(ОтчетОбъект.КонецПериода), КонецДня(ОтчетОбъект.КонецПериода), КонецПериода(Период, ОтчетОбъект.Интервал)), "ДФ=dd.MM.yyyy") + НСтр("ru=' (конец дня)';uk=' (кінець дня)'");
		ТиповыеОтчеты.УстановитьПараметр(ФормаОтчета.КомпоновщикНастроекПромежуточнаяРасшифровка, "НачалоПериода", Период);
		ТиповыеОтчеты.УстановитьПараметр(ФормаОтчета.КомпоновщикНастроекПромежуточнаяРасшифровка, "КонецПериода" , ?(КонецПериода(Период, ОтчетОбъект.Интервал) > КонецДня(ОтчетОбъект.КонецПериода), КонецДня(ОтчетОбъект.КонецПериода), КонецПериода(Период, ОтчетОбъект.Интервал)));
	КонецЕсли;
	...
	Если ИдентификаторОбъекта = "ОтчетОбъект.ДинамикаЗадолженностиПокупателей"
		...
		|	ХозрасчетныйОстатки.СуммаОстатокДтКт КАК Сумма,
		|	""" + НСтр("ru='Остаток ДтКт ';uk='Залишок ДтКт '") + """ + ХозрасчетныйОстатки.Счет.Код + "" """""" + ХозрасчетныйОстатки.Счет.Наименование + """""""" КАК СтрокаРасшифровки,
		|	NULL КАК КорСчет,
		|	NULL КАК Субконто1,
		|	NULL КАК Субконто2,
		...
		|			И (НЕ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоНалогамИПлатежам)))
...
Процедура ОбработкаРезультатаОтчетаСДиаграммой(ОтчетОбъект, Результат) Экспорт
	...
	МассивЗаголовков.Добавить(НСтр("ru='БУ Дт (нач. сальдо)';uk='БО Дт (поч. сальдо)'"));
	МассивЗаголовков.Добавить(НСтр("ru='БУ Кт (нач. сальдо)';uk='БО Кт (поч. сальдо)'"));
	МассивЗаголовков.Добавить(НСтр("ru='БУ Дт (оборот)';uk='БО Дт (оборот)'"));
	МассивЗаголовков.Добавить(НСтр("ru='БУ Кт (оборот)';uk='БО Кт (оборот)'"));
	МассивЗаголовков.Добавить(НСтр("ru='БУ Дт (кон. сальдо)';uk='БО Дт (кін. сальдо)'"));
	МассивЗаголовков.Добавить(НСтр("ru='БУ Кт (кон. сальдо)';uk='БО Кт (кін. сальдо)'"));
...
Функция ПолучитьПредставлениеНастройкиДляИстории(ОтчетОбъект) Экспорт 
	...
	Исключение
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомостьПоСчету"
			...
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.АнализСчета"
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.ОборотыСчета" Тогда 
			Префикс = НСтр("ru='По счету ';uk='По рахункові '") + ОтчетОбъект.Счет;
		КонецЕсли;
		...
		Если ИдентификаторОбъекта = "ОтчетОбъект.АнализСубконто" 
			...
			Префикс = НСтр("ru='По субконто ';uk='По субконто '") + ПредставлениеСубконто;	
		КонецЕсли;
		
		Если ИдентификаторОбъекта = "ОтчетОбъект.ОтчетПоПроводкам"
			ИЛИ ИдентификаторОбъекта = "ОтчетОбъект.СводныеПроводки" Тогда 
			Префикс = НСтр("ru='Проводки ';uk='Проводки'");
		КонецЕсли;
		
		Если ИдентификаторОбъекта = "ОтчетОбъект.ОборотноСальдоваяВедомость" Тогда 
			Префикс = НСтр("ru='Данные ';uk='Дані'");
		КонецЕсли;
		...
		СоответствиеПоказателей.Вставить("БУ"               , НСтр("ru='БУ';uk='БО'"));
		СоответствиеПоказателей.Вставить("НУ"               , НСтр("ru='НУ';uk='ПО'"));
		СоответствиеПоказателей.Вставить("Контроль"         , НСтр("ru='Разн.';uk='Різн.'"));
		СоответствиеПоказателей.Вставить("ВалютнаяСумма"    , НСтр("ru='Вал.';uk='Вал.'"));
		СоответствиеПоказателей.Вставить("Количество"       , НСтр("ru='Кол.';uk='Кіл.'"));
		СоответствиеПоказателей.Вставить("РазвернутоеСальдо", НСтр("ru='Разв.';uk='Розг.'"));
...
Функция ПолучитьДенежныеСредства(Период, Организация)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ДенежныеСредства.СуммаОстаток КАК Остаток
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Остатки(
	               |			&ГраницаПериод,
	               |			Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.Касса))
	               |			ИЛИ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.СчетаВБанках))
	               |			ИЛИ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте))
				   |			ИЛИ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДенежныеСредстваВПутиВИностраннойВалюте)),
	               |			,
	               |			Организация = &Организация) КАК ДенежныеСредства
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Хозрасчетный.Остатки(&ГраницаПериод, Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицами)), , Организация = &Организация) КАК Подотчет
	               |ИТОГИ
	               |	СУММА(Остаток)
	               |ПО
	               |	ОБЩИЕ";
...
Функция ПолучитьПродажи(НачалоПериода, КонецПериода, Организация)
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Доходы.СуммаОборотКт - ЕСТЬNULL(Вычеты.СуммаОборотДт, 0) КАК Сумма
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&Счета701_3), , Организация = &Организация, , ) КАК Доходы
	|	    ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&Счет704), , Организация = &Организация, КорСчет НЕ В ИЕРАРХИИ (&Счет643) , ) КАК Вычеты
	|	    ПО Истина
	|";
	...
	Счета701_3 = Новый СписокЗначений;
	Счета701_3.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииГотовойПродукции);
	Счета701_3.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииТоваров);
	Счета701_3.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииРаботИУслуг);
	Запрос.УстановитьПараметр("Счета701_3",   Счета701_3);
	Запрос.УстановитьПараметр("Счет704",   ПланыСчетов.Хозрасчетный.ВычетыИзДохода);
	Запрос.УстановитьПараметр("Счет643",   ПланыСчетов.Хозрасчетный.НалоговыеОбязательстваВсего);
...
Функция ПолучитьПоступлениеДенежныхСредств(НачалоПериода, КонецПериода, Организация)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
				   ...
				   | И СчетаКонтрагентов.Ссылка НЕ В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоНалогамИПлатежам))
	               |
	               |ИНДЕКСИРОВАТЬ ПО
	               |	Счет
	               |;
	               ...
	               |				И (НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДоходыОтРеализации)))
	               |				И (НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте)))
	               |				И (НЕ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДенежныеСредстваВПутиВИностраннойВалюте)))
	               ...
	               |				ИЛИ КорСчет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДоходыОтРеализации)),
...
//	СчетаДС.Добавить(ПланыСчетов.Хозрасчетный.ДенежныеСредстваВПутиВНациональнойВалюте);
//	СчетаДС.Добавить(ПланыСчетов.Хозрасчетный.ДенежныеСредстваВПутиВИностраннойВалюте);
...
Функция ПолучитьПрибыль(НачалоПериода, КонецПериода, Организация)
	
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               |	ЕСТЬNULL(Доходы.Доход, 0) - ЕСТЬNULL(Расходы.Расход, 0) КАК Сумма
	               |ИЗ
	               |	(ВЫБРАТЬ
	               |		СУММА(Доходы.СуммаОборотКт - ЕСТЬNULL(НДС.СуммаОборотДт, 0) - ЕСТЬNULL(Вычеты.СуммаОборотДт, 0)) КАК Доход
	               |	ИЗ
	               |		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&СчетаДоходов), , Организация = &Организация, , ) КАК Доходы
	               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&СчетаДоходов), , Организация = &Организация, КорСчет В ИЕРАРХИИ (&Счет643), ) КАК НДС
	               |			ПО Истина
				   |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&Счет704), , Организация = &Организация, , ) КАК Вычеты
				   |			ПО Истина) КАК Доходы,
	               |	(ВЫБРАТЬ
	               |		ХозрасчетныйОбороты.СуммаОборотДт КАК Расход
	               |	ИЗ
	               |		РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет В ИЕРАРХИИ (&СчетаРасходов), , Организация = &Организация, , ) КАК ХозрасчетныйОбороты) КАК Расходы";
	...
	СчетаДоходовПродажи = Новый СписокЗначений;
	СчетаДоходовПродажи.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииГотовойПродукции);
	СчетаДоходовПродажи.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииТоваров);
	СчетаДоходовПродажи.Добавить(ПланыСчетов.Хозрасчетный.ДоходОтРеализацииРаботИУслуг);
	
	СчетаДоходов = Новый СписокЗначений;
	
	СчетаДоходовПрочие = Новый СписокЗначений;
	
	ЗапросСубсчета70 = Новый Запрос;
	ЗапросСубсчета70.Текст = 
		"ВЫБРАТЬ
		|	Хозрасчетный.Ссылка
		|ИЗ
		|	ПланСчетов.Хозрасчетный КАК Хозрасчетный
		|ГДЕ
		|	Хозрасчетный.Родитель = &СчетРодитель
		|	И Хозрасчетный.Вид = &Пассивный
		|	И Хозрасчетный.Ссылка <> &СчетДоходыРозница
		|	И Хозрасчетный.Ссылка НЕ В ИЕРАРХИИ (&СчетаДоходовПродажи)";
		
	ЗапросСубсчета70.УстановитьПараметр("СчетРодитель",ПланыСчетов.Хозрасчетный.ДоходыОтРеализации);
	ЗапросСубсчета70.УстановитьПараметр("СчетДоходыРозница",ПланыСчетов.Хозрасчетный.ДоходОтРозничныхПродаж);
	ЗапросСубсчета70.УстановитьПараметр("Пассивный",ВидСчета.Пассивный);
	ЗапросСубсчета70.УстановитьПараметр("СчетаДоходовПродажи",СчетаДоходовПродажи);
	
	СчетаДоходов = СчетаДоходовПродажи.Скопировать();
	Субсчета70 = ЗапросСубсчета70.Выполнить().Выгрузить();
	Для каждого Субсчет Из Субсчета70 Цикл
		СчетаДоходовПрочие.Добавить(Субсчет.Ссылка);
		СчетаДоходов.Добавить(Субсчет.Ссылка);
	КонецЦикла; 
	СчетаДоходовПрочие.Добавить(ПланыСчетов.Хозрасчетный.ДругойОперационныйДоход);
	...
	Запрос.УстановитьПараметр("СчетаДоходовПродажи",   СчетаДоходовПродажи);
	Запрос.УстановитьПараметр("СчетаДоходовПрочие",   СчетаДоходовПрочие);
	Запрос.УстановитьПараметр("Счет704",   ПланыСчетов.Хозрасчетный.ВычетыИзДохода);
	Запрос.УстановитьПараметр("Счет643",   ПланыСчетов.Хозрасчетный.НалоговыеОбязательстваВсего);
	СчетаРасходов = Новый СписокЗначений;
	ИспользуемыеКлассыСчетовРасходов = Новый СписокЗначений;
	Если ЗначениеЗаполнено(Организация) Тогда
		ИспользуемыеКлассыСчетовРасходов.Добавить(БухгалтерскийУчет.ПолучитьИспользуемыеКлассыСчетовРасходов(КонецДня(КонецПериода), Организация));
	Иначе
		// Берем по всем организациям
		Выборка = Справочники.Организации.Выбрать();
		Пока Выборка.Следующий() Цикл
			ИспользуемыеКлассыСчетовРасходов.Добавить(БухгалтерскийУчет.ПолучитьИспользуемыеКлассыСчетовРасходов(КонецДня(КонецПериода), Выборка.Ссылка));
		КонецЦикла;
	КонецЕсли;
...
Функция ПолучитьАвансыВыданные(Период, Организация) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		РегистрБухгалтерии.Хозрасчетный.Остатки(
	               |			&Период,
	               |			Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасчетыПоВыданнымАвансам))
				   |					И (НЕ Счет В (&ИсключенныеСчета)),
	               |			&ВидыСубконтоКД,
	               |			
	               |				Организация = &Организация) КАК ДолгПокупателей
				   |	ГДЕ
	               |		ДолгПокупателей.СуммаОстаток > 0) КАК ВложенныйЗапрос
	               |ИТОГИ
	               |	СУММА(СуммаОстаток)
	               |ПО
	               |	ОБЩИЕ,
...
Функция ПолучитьОборотныеСредства(НачалоПериода, КонецПериода, Организация, СуммаНаНачало, СуммаНаКонец)
	...
	П1Н_30    = ПолучитьИтог(Счет30, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П1Н_31    = ПолучитьИтог(Счет31, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П1Н_33_3  = ПолучитьИтог(Счет33_3, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П1Н_33_4  = ПолучитьИтог(Счет33_4, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	
	П1К_30    = ПолучитьИтог(Счет30, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П1К_31    = ПолучитьИтог(Счет31, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П1К_33_3  = ПолучитьИтог(Счет33_3, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П1К_33_4  = ПолучитьИтог(Счет33_4, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	
	П1Н = П1Н_30 + П1Н_31 + П1Н_33_3 + П1Н_33_4;
	П1К = П1К_30 + П1К_31 + П1К_33_3 + П1К_33_4;
	
	
	// Краткосрочные финансовые вложения (порядок 2)
	П2Н_34	 = ПолучитьИтог(Счет34, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П2Н_35   = ПолучитьИтог(Счет35, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	
	П2К_34	 = ПолучитьИтог(Счет34, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П2К_35   = ПолучитьИтог(Счет35, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	...
	//
	// производственные запасы (порядок 5)
	П5Н_20	 = ПолучитьИтог(Счет20, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П5Н_22	 = ПолучитьИтог(Счет22, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	
	П5К_20	 = ПолучитьИтог(Счет20, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П5К_22	 = ПолучитьИтог(Счет22, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	
	П5Н = П5Н_20 + П5Н_22;
	П5К = П5К_20 + П5К_22;
	
	// текущие биологические активы (порядок 6)
	П6Н = ПолучитьИтог(Счет21, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П6К = ПолучитьИтог(Счет21, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	
	// Незавершенное производство (порядок 7)
	П7Н_23    = ПолучитьИтог(Счет23, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П7Н_25    = ПолучитьИтог(Счет25, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	
	П7К_23    = ПолучитьИтог(Счет23, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П7К_25    = ПолучитьИтог(Счет25, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	
	П7Н = П7Н_23 + П7Н_25;
	П7К = П7К_23 + П7К_25;
	
	// Готовая продукция (порядок 8)
	П8Н_26    = ПолучитьИтог(Счет26, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	П8Н_27    = ПолучитьИтог(Счет27, ВыборкаПоСчету, "СуммаНачальныйОстатокДт");
	
	П8К_26    = ПолучитьИтог(Счет26, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	П8К_27    = ПолучитьИтог(Счет27, ВыборкаПоСчету, "СуммаКонечныйОстатокДт");
	
	П8Н = П8Н_26 + П8Н_27;
	П8К = П8К_26 + П8К_27;
	
	// Товары (порядок 9)
	П9Н_28   = ПолучитьИтог(Счет28, ВыборкаПосчету, "СуммаНачальныйОстатокДт");
	П9Н_28_6 = ПолучитьИтог(Счет28_6, ВыборкаПосчету, "СуммаНачальныйОстатокДт");
	
	П9К_28	 = ПолучитьИтог(Счет28, ВыборкаПосчету, "СуммаКонечныйОстатокДт");
	П9К_28_6 = ПолучитьИтог(Счет28_6, ВыборкаПосчету, "СуммаКонечныйОстатокДт");
	
	П9Н = П9Н_28 - П9Н_28_6;
	П9К = П9К_28 - П9К_28_6;
	
	// Брак в производстве (порядок 12)
	П12Н = ПолучитьИтог(Счет24, ВыборкаПосчету, "СуммаНачальныйОстатокДт");
	П12К = ПолучитьИтог(Счет24, ВыборкаПосчету, "СуммаКонечныйОстатокДт");
	
	// Запасы  (порядок 4)
	П4Н = П5Н + П6Н + П7Н + П8Н + П9Н + П12Н;
	П4К = П5К + П6К + П7К + П8К + П9К + П12К;
	
	// Недостачи и потери (порядок 13)
	П13Н = ПолучитьИтог(Счет37_5, ВыборкаПосчету, "СуммаНачальныйОстатокДт");
	П13К = ПолучитьИтог(Счет37_5, ВыборкаПосчету, "СуммаКонечныйОстатокДт");
	
	ИтогоНаНачало = 0;
	ИтогоНаКонец  = 0;
	
	ИтогоНаНачало = ИтогоНаНачало + П1Н + П2Н + П5Н + П6Н + П7Н + П8Н + П9Н + П12Н + П13Н;
	ИтогоНаКонец  = ИтогоНаКонец + П1К + П2К + П5К + П6К + П7К + П8К + П9К + П12К + П13К;
...
Процедура ЗаполнитьСтрокуТаблицыОперативныхДанных(ПредшествующаяДата, ТекущаяДата, Организация, СтрокаТаблицы, Номер) Экспорт
	...
	ИначеЕсли Номер = 10 Тогда
		ЗаполнитьАвансыВыданные(СтрокаТаблицы, ТекущаяДата, Организация);	
	КонецЕсли;
...
Процедура ЗаполнитьДенежныеСредства(СтрокаТаблицы, ПредшествующаяДата, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель = НСтр("ru='Остаток денежных средств';uk='Залишок коштів'");
...
Процедура ЗаполнитьПродажи(СтрокаТаблицы, ПредшествующаяДата, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель = НСтр("ru='Продажи';uk='Продажі'");
...
Процедура ЗаполнитьПоступлениеДенежныхСредств(СтрокаТаблицы, ПредшествующаяДата, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель = НСтр("ru='Поступление денежных средств';uk='Надходження коштів'");
...
Процедура ЗаполнитьПрибыль(СтрокаТаблицы, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель        = НСтр("ru='Прибыль (убыток)';uk='Прибуток (збиток)'");
	УстановитьПериод(НачалоПериода, КонецПериода, ТекущаяДата, 1);
...
Процедура ЗаполнитьЗадолженностьПокупателей(СтрокаТаблицы, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель   = НСтр("ru='Задолженность покупателей';uk='Заборгованість покупців'");
...
Процедура ЗаполнитьПросроченнуюЗадолженностьПокупателей(СтрокаТаблицы, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель   = НСтр("ru='  в т. ч. просроченная';uk='  у т. ч.  прострочена'");
...
Процедура ЗаполнитьОборотныеСредства(СтрокаТаблицы, ТекущаяДата, Организация)
	...
	СтрокаТаблицы.Показатель   = НСтр("ru='Оборотные средства';uk='Обігові кошти'");
...
Процедура ЗаполнитьАвансыВыданные(СтрокаТаблицы, ТекущаяДата, Организация)
	
	СтрокаТаблицы.Показатель   = НСтр("ru='Авансы выданные';uk='Аванси, що видано'");
	СтрокаТаблицы.НачалоМесяца = ПолучитьАвансыВыданные(НачалоМесяца(ТекущаяДата), Организация);
	СтрокаТаблицы.Сегодня	   = ПолучитьАвансыВыданные(КонецДня(ТекущаяДата) + 1, Организация);
	СтрокаТаблицы.Изменение    = РассчитатьИзменение(СтрокаТаблицы.НачалоМесяца, СтрокаТаблицы.Сегодня);
...
Процедура ДобавитьЗаписьВТаблицуПромежуточныеДанные(Таблица, Параметры) Экспорт
	...
	Если ЗначениеЗаполнено(КорСчет) Тогда
		...
		КонецЕсли;	
	Иначе 
		СтрокаРасшифровки = НСтр("ru='Остаток ';uk='Залишок '") + БухВидРесурса + " " + Счет + " """ + Счет.Наименование + """";
	КонецЕсли;
...
Функция ПолучитьСохраненнуюНастройкуСчетовИсключаемыхИзРасчетаЗадолженности(Тип = 1) 
	...
	ИначеЕсли Тип = 3 Тогда
		НастраиваемыйОбъект = "ОтчетыДляРуководителя.СчетаИсключаемыеИзРасчетаДоходовРасходов";
	КонецЕсли;
...
Функция ПолучитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(Тип = 1) Экспорт
	...
	ИначеЕсли Тип = 3 Тогда
		...
		Возврат ПолучитьСчетаДРНеИспользуемыхПоУмолчанию();
		
	КонецЕсли;
	
КонецФункции

Функция ПолучитьСчетаДРНеИспользуемыхПоУмолчанию()
	...
	ЗапросСписок.УстановитьПараметр("СчетаРодитель"		,СчетаРодитель);
	
	Список = Новый СписокЗначений;
	Список.ЗагрузитьЗначения(ЗапросСписок.Выполнить().Выгрузить().ВыгрузитьКолонку("Счет"));
...
Процедура СохранитьСписокСчетовИсключаемыхИзРасчетаЗадолженности(Тип = 1, СписокСчетов) Экспорт
	...
	Если Тип = 1 Тогда
		НастраиваемыйОбъект = "ОтчетыДляРуководителя.СчетаИсключаемыеИзРасчетаЗадолженностиПокупателей";
		Наименование        = НСтр("ru='Счета исключаемые из расчета задолженности покупателей';uk='Рахунки заборгованості покупців, що виключаються із розрахунку'", Локализация.КодЯзыкаИнформационнойБазы());
	ИначеЕсли Тип = 2 Тогда
		Наименование        = Нстр("ru='Счета исключаемые из расчета задолженности поставщикам';uk='Рахунки заборгованості постачальникам, що виключаються із розрахунку'", Локализация.КодЯзыкаИнформационнойБазы());
		Наименование        = Нстр("ru='Счета исключаемые из расчета задолженности поставщикам';uk='Рахунки заборгованості постачальникам, що виключаються із розрахунку'", Локализация.КодЯзыкаИнформационнойБазы());
	ИначеЕсли Тип = 3 Тогда
		НастраиваемыйОбъект = "ОтчетыДляРуководителя.СчетаИсключаемыеИзРасчетаДоходовРасходов";
		Наименование        = Нстр("ru='Счета исключаемые из расчета Доходов/Расходов';uk='Рахунки, що виключають із розрахунку Доходів/Витрат'", Локализация.КодЯзыкаИнформационнойБазы());
	КонецЕсли;
...
Функция СформироватьСтрокуОграниченийПоДатамДляФормы(Знач ДатаНач, Знач ДатаКон) Экспорт
	...
	Если ДатаНач = '00010101000000' И ДатаКон = '00010101000000' Тогда
		...
		ОписаниеПериода     = НСтр("ru='Период не установлен';uk='Період не встановлений'");
	...
	Иначе
		...
		Иначе
			...
			Иначе
				ОписаниеПериода = НСтр("ru='Неправильно задан период!';uk='Неправильно заданий період!'")
			КонецЕсли;
