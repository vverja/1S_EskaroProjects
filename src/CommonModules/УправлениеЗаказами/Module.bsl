...
Функция ИспользоватьЗаказВТабличнойЧасти(ДокументВидОперации, ДоговорКонтрагента, ВидОперации = "Реализация") Экспорт
	...
	Если УказаниеЗаказов Тогда
		Если ВидОперации = "Реализация" Тогда
			Если ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Оборудование Тогда
				ИспользоватьКолонкуЗаказ.ИспользоватьЗаказВТабЧасти = Ложь;						
			ИначеЕсли (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссия)
			  //Смартис Лиманчук начало 01.10.2012
			  ИЛИ (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.ПродажаКомиссияНал)
			  //Смартис Лиманчук окончание 01.10.2012
			  ИЛИ (ДокументВидОперации = Перечисления.ВидыОперацийРеализацияТоваров.Брак) Тогда
				...
				КонецЕсли;
			КонецЕсли;
		ИначеЕсли ВидОперации = "РеализацияСчет" Тогда
			Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
				ВедениеВзаиморасчетов = ДоговорКонтрагента.ВедениеВзаиморасчетов;
		...
		ИначеЕсли ВидОперации = "ВозвратОтПокупателя" Тогда
			Если ДокументВидОперации = Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.Оборудование Тогда
				ИспользоватьКолонкуЗаказ.ИспользоватьЗаказВТабЧасти = Ложь;						
			Иначе
				Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
					ВедениеВзаиморасчетов = ДоговорКонтрагента.ВедениеВзаиморасчетов;
		...
		ИначеЕсли ВидОперации = "Поступление" Тогда
			Если ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия
				//Смартис Лиманчук начало 01.10.2012
				ИЛИ ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссияНал
				//Смартис Лиманчук окончание 01.10.2012
				ИЛИ ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование
				//Смартис Лиманчук начало 05.12.2012
				ИЛИ ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОборудованиеНал
				//Смартис Лиманчук окончание 05.12.2012
				ИЛИ ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.БланкиСтрогогоУчета Тогда
				...
				Если ЗначениеЗаполнено(ДоговорКонтрагента) Тогда
					...
					КонецЕсли;
				КонецЕсли;
			ИначеЕсли ДокументВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку Тогда	
				ИспользоватьКолонкуЗаказ.ОбязательноеЗаполнениеЗаказаВТабЧасти = Ложь;
				ИспользоватьКолонкуЗаказ.ИспользоватьЗаказВТабЧасти = Ложь;
			КонецЕсли;
...
//
Функция ИспользоватьЗаказыНаПроизводство(Сообщать = Ложь) Экспорт
...
КонецФункции // ИспользоватьЗаказыНаПроизводство()
...
Процедура НачалоВыбораДокументаЗаказа(ДокументОбъект, ФормаДокумента, ЭлементФормы, 
	                   СтандартнаяОбработка, СтруктураОтбора, ИмяТабличнойЧасти, ДопПараметры = Неопределено, ИмяКолонки = Неопределено) Экспорт
	...
	Если Не УправлениеЗаказами.ИспользоватьЗаказыНаПроизводство() Тогда
		...
		Пока Индекс >= 0 Цикл
			Если ТипЗаказНаПроизводство = МассивТипов[Индекс] Тогда
				МассивТипов.Удалить(Индекс);
				Прервать;
			КонецЕсли;
			Индекс = Индекс - 1;
...
Процедура ЗаполнитьЗаказПокупателяВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	Для каждого СтрокаТЧ Из ДокументОбъект.Товары Цикл
		Если СтрокаТЧ.ЗаказПокупателя <> ЗаказИзШапки Тогда
...
Процедура ЗаполнитьЗаказПоставщикуВТЧ(ДокументВидОперации, ДокументОбъект, ВидОперации) Экспорт
	...
	Для каждого СтрокаТЧ Из ДокументОбъект.Товары Цикл
		Если СтрокаТЧ.ЗаказПоставщику <> ЗаказИзШапки Тогда
...
Процедура ПроверитьРезервированиеТарыПодЗаказСОбособленнымУчетом(ДокументОбъект,ИмяРевизитаЗаказ="", ЗаказВШапке=ложь, ИмяРеквизитаСклад="", Отказ, Заголовок) Экспорт
	Перем ОбособленныйУчетПоЗаказу;  //Признак ведения особленного учета по заказу
	...
	Для Каждого СтрокаТаблицы Из ДокументОбъект.ВозвратнаяТара Цикл
		//возможно для документа КорректировкаЗаказаПокупателя. Выполняется снятие резерва - проверка не требуется
		Если СтрокаТаблицы.Количество<0 Тогда 
			Продолжить;
		КонецЕсли;
		Если ЗаказВШапке Тогда
			//проверка заполнения склада. Если склад указан - это признак выполнения резервирования под заказ с обособленным учетом
		...
		Иначе
			...
			//Если заказ не указан, резервирование не выполняется
			Если не ЗначениеЗаполнено(Заказ) Тогда
				Продолжить;
			КонецЕсли;
...
//
Функция ПолучитьСтруктуруОтбораПоЗаказам(ДокОбъект) Экспорт
	СтруктураОтбора = Новый Структура;
	СтруктураОтбора.Вставить( "Организация",        ДокОбъект.Организация);
	СтруктураОтбора.Вставить( "Контрагент",         ДокОбъект.Контрагент);
	Если НЕ ДокОбъект.ДоговорКонтрагента.ВедениеВзаиморасчетов = Перечисления.ВедениеВзаиморасчетовПоДоговорам.ПоСчетам Тогда
		Если ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ВПереработку Тогда
			СтруктураОтбора.Вставить( "ВидОперации", Перечисления.ВидыОперацийЗаказПоставщику.Переработка);
		ИначеЕсли ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.Оборудование Тогда
			СтруктураОтбора.Вставить( "ВидОперации", Перечисления.ВидыОперацийЗаказПоставщику.Оборудование);
		//Смартис Лиманчук начало 05.12.2012
		ИначеЕсли ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ОборудованиеНал Тогда
			СтруктураОтбора.Вставить( "ВидОперации", Перечисления.ВидыОперацийЗаказПоставщику.Оборудование);
		//Смартис Лиманчук окончание 05.12.2012
		//Смартис Лиманчук начало 02.10.2012
		//ИначеЕсли ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия Тогда
		ИначеЕсли (ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссия ИЛИ ДокОбъект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ПокупкаКомиссияНал) Тогда
...
//
//
Функция ВыбратьЗаказИзДокументаОснования (Параметры) Экспорт
...
Процедура ВывестиПризнакОбособленныйУчетВФормуСписка(Элемент, ОформленияСтрок, соотвТипы) Экспорт

	МассивДокументов = новый Массив;
	Соответствие = новый Соответствие;
	Для каждого Строка из ОформленияСтрок цикл
		Строка.Ячейки.ОбособленныйУчет.ОтображатьФлажок = Истина;
        Строка.Ячейки.ОбособленныйУчет.Флажок = Ложь;
	...
	Если соотвТипы.Количество()>0 Тогда
		Для каждого элт из СоотвТипы цикл
			Запрос.Текст = Запрос.Текст+"
			|ОБЪЕДИНИТЬ ВСЕ
			|ВЫБРАТЬ 
			|Ссылка,
			|ВЫРАЗИТЬ(Ссылка."+элт.Значение+".ДоговорКонтрагента.ОбособленныйУчетТоваровПоЗаказамПокупателей КАК Булево) КАК ОбособленныйУчет
			|ИЗ Документ."+элт.Ключ+"
			|ГДЕ Ссылка в (&МассивДокументов) И ВЫРАЗИТЬ("+элт.Значение+".ДоговорКонтрагента.ОбособленныйУчетТоваровПоЗаказамПокупателей КАК Булево)";
		КонецЦикла;
	КонецЕсли;
...
//
Функция ПолучитьПоследнийЗаказПокупателяИлиИзменениеЗаказаПокупателя(ЗаказПокупателя, НаДату = Неопределено) Экспорт
	...
	Если НаДату = Неопределено Тогда
		УсловиеДатаЗаказа = "";
	Иначе
		УсловиеДатаЗаказа = "И ДокЗаказ.Дата < &НаДату";
...
КонецФункции // ПолучитьПоследнийЗаказПокупателяИлиИзменениеЗаказаПокупателя
...
Функция ПодготовитьТаблицуДляЗакрытияРазмещения(ДокСсылка,МоментВремени,СписокЗаказов,РазмещенияВЗаказе,РазмещенияЗаказа, Знак = "") Экспорт
...
//		МоментВремени - момент времени документа для определения остатков
//		СписокЗаказов - массив заказов для отбора
Функция ПодготовитьТаблицуДляЗакрытияЗаказовПокупателей(ДокСсылка,МоментВремени,СписокЗаказов, Знак = "") Экспорт
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументСсылка",            ДокСсылка);
	Запрос.УстановитьПараметр("МоментДокумента",           МоментВремени);
	Запрос.УстановитьПараметр("СписокЗаказов",             СписокЗаказов);
...
//		МоментВремени - момент времени документа для определения остатков
//		СписокЗаказов - массив заказов для отбора
Функция ПодготовитьТаблицуДляЗакрытияЗаказовПоставщикам(ДокСсылка,МоментВремени,СписокЗаказов, Знак = "") Экспорт
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("ДокументСсылка",            ДокСсылка);
	Запрос.УстановитьПараметр("МоментДокумента",           МоментВремени);
	Запрос.УстановитьПараметр("СписокЗаказов",             СписокЗаказов);
...
//			КолОстаток    - количество неоказанных услуг по заказу
//
Функция ОстаткиОборудованияПоЗаказуПоставщику( Заказ, Договор, Знач КонДата, Знач СтатусПартии = Неопределено, ДопПараметры = Неопределено) Экспорт
...
КонецФункции // ОстаткиОборудованияПоЗаказуПоставщику()
...
Процедура ДвижениеПоВнутреннимЗаказам( ДокОбъект, ТабИсходная, ДопПараметры, Отказ, Заголовок) Экспорт
	...
	Для Каждого СтрокаДок Из ТабПоЗаказам Цикл
		...
		Если Не ТипЗнч(Заказ.Ссылка) = Тип("ДокументСсылка.ВнутреннийЗаказ") Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьТабЧастьТоварыПоЗаказу(ДокСсылка, ТабЧасть, Заказ, ТабОстатки, ДопПараметры = Неопределено, ОчищатьСтроки = Ложь) Экспорт
	...
	Для Каждого СтрокаОстатков Из ТабОстатки Цикл
		...
		Если ФлагЕстьСтавкаНДС Тогда
			НоваяСтрока.СуммаНДС = Ценообразование.РассчитатьСуммуНДС(
				НоваяСтрока.Сумма,
				ФлагУчитыватьНДС,
...
Процедура ЗаполнитьТабЧастьТоварыПоЗаказуПоставщику(ДокОбъект, ТабЧасть, ТабОстатки, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого ДобавленнаяСтрока Из СоотвСтрок Цикл
		...
		Если НЕ ФлагЭтоТара Тогда
			...
			ДобавленнаяСтрока.Ключ.СуммаНДС = Ценообразование.РассчитатьСуммуНДС(
				ДобавленнаяСтрока.Ключ.Сумма,
				ФлагУчитыватьНДС,
				ФлагСуммаВключаетНДС,
				Ценообразование.ПолучитьСтавкуНДС(ДобавленнаяСтрока.Ключ.СтавкаНДС));
			КонецЕсли;
...
Процедура ЗаполнитьТабЧастьТоварыПоВнутреннемуЗаказу(ДокСсылка, ТабЧасть, ТабОстатки, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаОстатков Из ТабОстатки Цикл
		...
		НоваяСтрока.Номенклатура               = СтрокаОстатков.Номенклатура;
		НоваяСтрока.ХарактеристикаНоменклатуры = СтрокаОстатков.ХарактеристикаНоменклатуры;
		НоваяСтрока.Количество                 = СтрокаОстатков.КолОстаток * СтрокаОстатков.ЕдиницаХраненияКоэффициент / СтрокаОстатков.Коэффициент;
		НоваяСтрока.ЕдиницаИзмерения           = СтрокаОстатков.ЕдиницаИзмерения;
		НоваяСтрока.Коэффициент                = СтрокаОстатков.Коэффициент;
...
//
// Параметры
//  ФлагРаспределятьНаБСУ – Булево – Используеться для документов с ТЧ БланкиСтрогогоУчета. 
//								   Если в добавляемой в ТЧ строке есть бланк строгого учета(БСУ),
//								   тогда необходимо поместить его в ТЧ БланкиСтрогоУчета.
Процедура ЗаполнитьОстаткамиТоваровСРезервомПоВнутреннемуЗаказу( ЭтотОбъект, Заказ, ТабЧасть, Склад, Знач КонДата, СтатусыПартий = Неопределено, ФлагРаспределятьНаБСУ = Ложь) Экспорт
	
	МетаданныеДокумента = ЭтотОбъект.Метаданные(); 
	РаспределитьНаБСУ = ФлагРаспределятьНаБСУ И (МетаданныеДокумента.ТабличныеЧасти.Найти("БланкиСтрогогоУчета") <> Неопределено);
	...
	|	ВнутренниеЗаказыОстатки.Номенклатура                                     КАК Номенклатура, " 
	+ ?(РаспределитьНаБСУ, "
							|	ВнутренниеЗаказыОстатки.Номенклатура.ВидНоменклатуры.БланкСтрогогоУчета",
							"
							|	Ложь") + " КАК БСУ, "
	+ "			
	|	ВнутренниеЗаказыОстатки.ХарактеристикаНоменклатуры                       КАК ХарактеристикаНоменклатуры,
			...
			Пока ВыборкаПоЕдиницамИзмерения.Следующий() цикл
				...
				Пока Выборка.Следующий() цикл
					...
					Если ВРезерве>0 Тогда
						// Если текущая номенклатура БСУ - добавить в ТЧ БСУ, а не в переданую ТЧ.
						НоваяСтрока = ?(Выборка.БСУ, ЭтотОбъект.БланкиСтрогогоУчета.Добавить(), ТабЧасть.Добавить());
						ЗаполнитьЗначенияСвойств(НоваяСтрока, Выборка);
						НоваяСтрока.Количество                   = ВРезерве;
						НоваяСтрока.ДокументРезерва 			 = Заказ;
						НоваяСтрока.ВнутреннийЗаказ 			 = Заказ;
						//Смартис Лиманчук начало 27.04.2012
						Если ЗначениеЗаполнено(Заказ.ДокументОснование) И (ТипЗнч(Заказ.ДокументОснование)=Тип("ДокументСсылка.ЗаказПокупателя")) Тогда
							НоваяСтрока.ДокументРезерва = Заказ.ДокументОснование;
						КонецЕсли;
						...
						Если Не ФлагТары ИЛИ Выборка.БСУ Тогда
							Если ЕстьКачество И НЕ Выборка.БСУ Тогда
								НоваяСтрока.Качество = Справочники.Качество.Новый;
							КонецЕсли;
...
//		ТабОстатки - таблица неотгружененых товаров по заказу
//
Процедура ЗаполнитьТабЧастьОборудованиеПоЗаказуПоставщику(ДокОбъект, ТабЧасть, ТабОстатки, ДопПараметры = Неопределено) Экспорт
	...
	Для Каждого СтрокаОстатков Из ТабОстатки Цикл
		...
		КонецЕсли;
		НоваяСтрока.Цена                       = СтрокаОстатков.Цена * НоваяСтрока.Коэффициент;
		НоваяСтрока.СтавкаНДС = СтрокаОстатков.СтавкаНДС;
		
		ОбработкаТабличныхЧастей.РассчитатьКоличествоМестТабЧасти( НоваяСтрока, ДокОбъект); 
		ОбработкаТабличныхЧастей.РассчитатьСуммуТабЧасти         ( НоваяСтрока, ДокОбъект); 
...
КонецПроцедуры // ЗаполнитьТабЧастьОборудованиеПоЗаказуПоставщику()
...
//
Функция ПолучитьДанныеДокументаПоСтруктуреКолонок(Колонки, ВидДокумента, СтруктураПолей, МассивДокументов) Экспорт
	...
	Для Каждого Реквизит Из СтруктураПолей Цикл
		Если Колонки[Реквизит.Ключ].Видимость Тогда			
			ТекстЗапроса  = ТекстЗапроса + Символы.ПС+", " + Реквизит.Значение + " КАК " + СокрЛП(Реквизит.Ключ);
		КонецЕсли;
	КонецЦикла;
...
КонецФункции //ПолучитьДанныеЗаказаПоСтруктуреКолонок
