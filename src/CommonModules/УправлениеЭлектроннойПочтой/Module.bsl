...
//
Процедура НаписатьПисьмоОбъекту(Объект) Экспорт
	...
	Иначе
		...
		НаписатьПисьмо(глЗначениеПеременной("глТекущийПользователь"),СтруктураНовогоПисьма,,, (НЕ ПустаяСтрока(Адрес)));
	КонецЕсли; 
...
Процедура ОбработкаСобытияOnClickПоляHTML(ТекущийПользователь, Элемент, pEvtObj, УчетнаяЗапись = Неопределено, ГруппаУчетнойЗаписи = Неопределено, ФормаВладелец = Неопределено, ЭлектронноеПисьмо = Неопределено) Экспорт
	...
	Если НЕ ПустаяСтрока(href) Тогда
		...
		Если (СтрЧислоВхождений(href, "ИмяФайлаРедактированияХТМЛТекста") > 0) И
			(((ТипЗнч(ЭлектронноеПисьмо) = Тип("ДокументСсылка.ЭлектронноеПисьмо")) И Не ЭлектронноеПисьмо.Пустая())
				ИЛИ 
			((ТипЗнч(ЭлектронноеПисьмо) = Тип("ДокументОбъект.ЭлектронноеПисьмо")) И Не ЭлектронноеПисьмо.ЭтоНовый())) Тогда
...
Процедура ПерейтиПоСсылкеИзХТМЛПоля(Знач ВыделенныйТекст, ТекущийПользователь, ФормаВладелец, УчетнаяЗапись) Экспорт
	...
	Иначе
		...
		Попытка
			ЗапуститьПриложение("explorer " + СокрЛП(ВыделенныйТекст));
		Исключение
			ОбщегоНазначения.СообщитьОбОшибке(ОписаниеОшибки());
		КонецПопытки;
...
//  
Функция ОткрытьФайлMSG(ОбъектФайла) Экспорт
...
Функция ПолучитьДоступныеУчетныеЗаписи(Пользователь) Экспорт
	...
	Если ПравоДоступа("Чтение", Метаданные.Справочники.УчетныеЗаписиЭлектроннойПочты) Тогда
		...
		Иначе
			РольПолныеПрава = ПользователиИнформационнойБазы.НайтиПоИмени(СокрЛП(Пользователь.Код)).Роли.Содержит(Метаданные.Роли.ПолныеПрава);
		КонецЕсли;
...
Функция ИсключитьПрефиксыТемыПисьма(Знач Тема) Экспорт
	...
	СписокВозможныхПрефиксов.Добавить("НА");
	СписокВозможныхПрефиксов.Добавить(">>");
...
//
Функция ПолучитьПредставлениеПолучателей(КомуТЧ) Экспорт
