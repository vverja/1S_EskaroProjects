...
Процедура ВыгрузитьИнформациюОБанке(ОбъектXML, РольБанка, Банк)
	...
	ЗаписатьТекстовойУзел(ОбъектXML, "МФО", Банк.Код);
...
Процедура ВыгрузитьЮрАдресОрганизации(ОбъектXML, Организация)
	...
	Если Выборка.Следующий() Тогда
		...
		Если ЗначениеЗаполнено(Выборка.Поле1) Тогда
			...
			Исключение
				АдресУкраинский = Ложь;
			КонецПопытки;
			
		КонецЕсли;
			
		// украинский адрес
		Если АдресУкраинский Тогда
...
Процедура ВыгрузитьКонтрагентаДок(ОбъектXML, Контрагент, Роль = Неопределено, СтруктурнаяЕдиница = Неопределено, ИмяГруппыВыгрузки = "Контрагент")
	...
	СведенияОПродавце   = УправлениеКонтактнойИнформацией.СведенияОЮрФизЛице(Контрагент, ТекущаяДата());
	ИННОрганизации		= ФормированиеПечатныхФорм.ОписаниеОрганизации(СведенияОПродавце, "ИНН,", Ложь);
	ЗаписатьТекстовойУзел(ОбъектXML, "ИНН", ИННОрганизации);
...
Процедура ВыгрузитьРеализациюТоваровУслуг(ОбъектXML, СписокДок, ВыводитьИнформационныесообщения)
	...
	|	"""" КАК СерияНоменклатурыСертификат,
...
Процедура ОтправитьПоПочте() Экспорт
	...
	ИначеЕсли ТипВыгрузки = "Прайс" Или ТипВыгрузки = "ПрайсCML_1" Тогда
