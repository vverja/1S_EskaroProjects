...
Процедура ЗаполнитьПостроительОтчетаПоЦенамНоменклатуры() Экспорт
	...
	ПостроительОтчета.Параметры.Вставить("ЕстьРисунки", ПечатьИзображений);
	...
	|	ВЫБОР
	|			КОГДА &ЕстьРисунки
	|		ТОГДА ЗапросПрайс.Номенклатура.ОсновноеИзображение
	|		ИНАЧЕ """"
	| 	КОНЕЦ КАК Рисунок
	...
	|	Штрихкоды.ЕдиницаИзмерения //КАК ХарактеристикаНоменклатурыШтрихКода
	|ИЗ
	...
	|	ЗапросПрайс.Номенклатура,
	|	ЗапросПрайс.ХарактеристикаНоменклатуры
	|ИТОГИ СУММА(Цена), МИНИМУМ(Валюта), МИНИМУМ(ЕдиницаИзмерения), МИНИМУМ(Остаток),
	...
	"Тип цен", "Номенклатура", "Характеристика номенклатуры", "Ед. измерения", "Остаток", "Ценовая группа", "Штрих-код");
...
Процедура ВывестиСтрокуПрайса(Выборка, НомерГруппировки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ОтображатьИерархиюВЦвете Тогда
			...
			Если ПечатьИзображений Тогда
				СекцияРисунок.Область(1, 1, 1, 1).ЦветФона = ЦветФона;
			КонецЕсли;
		...
		Если ЭтоНижнийУровень Тогда
			...
			КонецЕсли;
			Если ПечатьИзображений Тогда
				ОсновноеИзображение = Выборка.Рисунок.Хранилище.Получить();
				Если ЗначениеЗаполнено(ОсновноеИзображение) Тогда
					//СекцияРисунок.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Картинка);
					Рис = мТабДок.Рисунки.Добавить(ТипРисункаТабличногоДокумента.Картинка);
					Индекс = мТабДок.Рисунки.Индекс(Рис);
					//СекцияРисунок.Рисунки[0].Картинка = ОсновноеИзображение;
					мТабДок.Рисунки[Индекс].Картинка = ОсновноеИзображение;
					ОбластьРисунка = мТабДок.Область(мТабДок.ВысотаТаблицы, 3, мТабДок.ВысотаТаблицы, 3);
					ОбластьРисунка.ВысотаСтроки = 100;
					мТабДок.Рисунки[Индекс].Расположить(ОбластьРисунка);					
					мТабДок.Рисунки[Индекс].РазмерКартинки=РазмерКартинки.АвтоРазмер;	
				КонецЕсли; 
				мТабДок.Присоединить(СекцияРисунок);		
			КонецЕсли; 
			...
			Пока ВыборкаТипЦен.Следующий() Цикл
				...
				СекцияТипЦен.Параметры.Цена = ОбщегоНазначения.ФорматСумм4(Ценообразование.ОкруглитьЦену(ВыборкаТипЦен.Цена, ВыборкаТипЦен.ПорядокОкругления, ВыборкаТипЦен.ОкруглятьВБольшуюСторону)) + СтрВалюта;
		...
		Иначе
			...
			Если ЭтоНеИтогПоИерархии Тогда
				ВывестиСтрокуПрайса(Выборка.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, мСоответствиеГруппировок[НомерГруппировки + 1]), НомерГруппировки + 1);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ПечатьПрайсЛиста(ДокументРезультат) Экспорт
	Если ПечатьИзображений Тогда
		РезультатЗапроса = ВыполнитьЗапрос(Новый Структура("НетХарактеристик, ПорядокОкругления, ОкруглятьВБольшуюСторону, Цена, Рисунок"));
	Иначе
	...
	Если ПечатьИзображений Тогда
		Макет = ЭтотОбъект.ПолучитьМакет("МакетПрайсаИзображения");
	Иначе
	...
	//Добавляем рисунки
	Если ПечатьИзображений Тогда
		Секция = Макет.ПолучитьОбласть("Шапка|Изображение");
		мТабДок.Присоединить(Секция);	
	КонецЕсли;
	...
	Если ПечатьИзображений Тогда
		мСписокРисунки = Макет.ПолучитьОбласть("Список|Изображение");	
	КонецЕсли; 	
	...
	ВывестиСтрокуПрайса(РезультатЗапроса.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам, мСоответствиеГруппировок[0]), 0);
