...
// Возвращаемое значение:
//   <Строка>   – <Строка запроса для расчета свободного остатка>
Функция ПолучитьСтрокуЗапросаСвободныеОстаткиУПП(Знач СтруктураРегистровОстатков, ТекстУсловияКачества, ПолучатьНоменклатуры) Экспорт

	СтрокаЗапроса = "";
	СоответствиеИмяРегистаУсловие = Новый Соответствие();
	СоответствиеИмяРегистаУсловие.Вставить("ТоварыНаСкладах"  , ЭтотОбъект.мУсловиеНоменклатураСклад + ТекстУсловияКачества);
	СоответствиеИмяРегистаУсловие.Вставить("ТоварыВРознице"   , ЭтотОбъект.мУсловиеНоменклатураСклад + ТекстУсловияКачества);
	...
	СоответствиеИмяРегистаУсловие.Вставить("СвободныеОстатки" , ЭтотОбъект.мУсловиеНоменклатураСклад + ТекстУсловияКачества);
...
Процедура СтруктураЗапросаОстаткиНоменклатурыСХарактеристикамиСвободныеОстатки(ДатаЗапроса, СписокПараметровЗапроса, ПоказыватьОстатки, РазворачиватьДоКачества) Экспорт
	...
	//Получаем Текст запроса свободных остатков
	Текст = Текст + ПолучитьСтрокуЗапросаСвободныеОстаткиУПП(Новый Структура("СвободныеОстатки,ТоварыВНТТ"), ТекстУсловияКачества, Ложь);
...
Функция ПолучитьСтрокуУсловия(ПоСправочнику, ИмяУсловия, Склад = Неопределено, Организация = Неопределено) Экспорт
	...
	ИначеЕсли ИмяУсловия = "НоменклатураСкладОрганизация" Тогда
		мПревышатьОстаткиПоОрганизации = Ложь;
