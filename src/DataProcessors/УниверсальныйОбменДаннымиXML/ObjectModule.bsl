...
Перем ВнешняяОбработкаОбработчиковСобытий Экспорт; // Объект "ВнешниеОбработкиМенеджер" для вызова экспортных процедур обработчиков при отладке загрузки/выгрузки
...
Перем мМакетОбщиеПроцедурыФункции;  // Текстовый документ с комментариями, глобальными переменными и обертками общих процедур и функций
...
Перем мСоотвествиеПустыхЗначенийТипов;
Перем мСоотвествиеОписаниеТипов;
...
Функция одОписаниеТипа(ЗначениеТипа) Экспорт
	...
	ОписаниеТипов = мСоотвествиеОписаниеТипов[ЗначениеТипа];
	...
	Если ОписаниеТипов = Неопределено Тогда
		...
		мСоотвествиеОписаниеТипов.Вставить(ЗначениеТипа, ОписаниеТипов);
...
Функция одПолучитьПустоеЗначение(Тип) Экспорт
	...
	ПустоеЗначениеТипа = мСоотвествиеПустыхЗначенийТипов[Тип];
	
	Если ПустоеЗначениеТипа = Неопределено Тогда
		
		ПустоеЗначениеТипа = одОписаниеТипа(Тип).ПривестиЗначение(Неопределено);	
		
		мСоотвествиеПустыхЗначенийТипов.Вставить(Тип, ПустоеЗначениеТипа);
...
Функция ЗаписатьИнформациюОбОшибкеОбработчикаОчисткиДанных(КодСообщения, СтрокаОшибки, ИмяПравилаОчисткиДанных, Объект = "", ИмяОбработчика = "") Экспорт
...
Функция ЗаписатьИнформациюОбОшибкеВыгрузкиОбработчикаПКО(КодСообщения, СтрокаОшибки, ПКО, Источник = "", ИмяОбработчика) Экспорт
...
Функция ЗаписатьИнформациюОбОшибкеОбработчикиПВД(КодСообщения, СтрокаОшибки, ИмяПравила, Объект = Неопределено, ИмяОбработчика) Экспорт
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ ЭКСПОРТА ОБРАБОТЧИКОВ И АЛГОРИТМОВ В TXT-ФАЙЛ ИЗ ПРАВИЛ ОБМЕНА
...
//          удалость прочесть правила обмена
//  
Процедура ВыгрузитьОбработчикиСобытий(Отказ) Экспорт
	...
	#Если Клиент Тогда
		Если ЭтоИнтерактивныйРежим Тогда
			...
			Если ФлагОшибки Тогда
				Предупреждение("При выгрузке обработчиков были обнаружены ОШИБКИ!");
				
			Иначе
				
				Предупреждение("Обработчики успешно выгружены!");
			КонецЕсли;
...
// Очищет переменные со структурой правил обмена
//
// Параметры:
//  Нет.
//  
...
//  Если режми отладки алгоритмов - "алгоритмы отлаживать как процедуры", то в конструктор добавляется структура "Алгоритмы".
...
Процедура ДобавитьВПотокКомментарий(Результат, ИмяОбласти, ВывестиКомментарий = Истина)
	...
	КомментарийИзМакета = Сред(КомментарийИзМакета, 1, СтрДлина(КомментарийИзМакета)); //исключаем последний перевод строки
...
//////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ И ФУНКЦИИ ПОЛУЧЕНИЯ ПОЛНОГО КОДА АЛГОРИТМОВ С УЧЕТОМ ИХ ВЛОЖЕННОСТИ
...
Процедура ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, ИмяАлгоритма, СтрокаПрефикса, Заголовок) 
	...
	ЗаголовокАлгоритма = "//============================ " + Заголовок + " """ + ТРег(ИмяАлгоритма) + """ ============================";
...
//  АлгоритмыОбработчика - Массив- содержит имена алгоритмов, которые вызываются из заданного обработчика
//  
Процедура ПолучитьАлгоритмыСтрокиОбработчика(СтрокаОбработчика, АлгоритмыОбработчика)
	...
	СимволНачальный = Найти(СтрокаОбработчика, ШаблонПоиска);
	...
	Если Найти(СтрокаОбработчикаДоВызоваАлгоритма, "//") <> 0  Тогда 
	...
	СимволКонечный = Найти(СтрокаОбработчика, ")") - 1;
...
// 
Функция ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Обработчик, Знач СтрокаПрефикса = "", АлгоритмВладелец, МассивЗапрошенныхЭлементов) 
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, "{НАЧАЛО АЛГОРИТМА}");
		...
		Если АлгоритмыОбработчика.Количество() <> 0 Тогда //В этой строке есть вызовы алгоритмов
			...
			Для Каждого Алгоритм ИЗ АлгоритмыОбработчика Цикл
				...
				Если МассивЗапрошенныхЭлементов.Найти(ВРег(Алгоритм)) <> Неопределено Тогда //Рекурсивный вызов алгоритма
					...
					ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, Алгоритм, СтрокаПрефиксаДляВложенногоКода, "{!!! РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА !!!}");
					
					СтрокаОператора = "ВызватьИсключение ""РЕКУРСИВНЫЙ ВЫЗОВ АЛГОРИТМА: "+ ТРег(Алгоритм) + """;";
				...
				Иначе
					...
					ОбработчикНовый = ОбработчикНовый + ЗаменитьВызовыАлгоритмовКодомЭтихАлгоритмовВОбработчике(Алгоритмы[Алгоритм], СтрокаПрефиксаДляВложенногоКода, Алгоритм, СкопироватьМассив(МассивЗапрошенныхЭлементов));
	...
	ЗаписатьЗаголовокБлокаАлгоритма(ОбработчикНовый, АлгоритмВладелец, СтрокаПрефикса, "{КОНЕЦ АЛГОРИТМА} ");
...
Функция ПолучитьАлгоритмыОбработчика(Обработчик)
	...
	Для Индекс = 1 По СтрЧислоСтрок(Обработчик) Цикл
		...
		Если Лев(СтрокаОбработчика, 2) = "//" Тогда //Строка закомментирована, её пропускаем..
			Продолжить;
		КонецЕсли;
...
Функция ПолучитьПрефиксДляВложенногоАлгоритма(СтрокаОбработчика, СтрокаПрефикса)
	...
	НомерПозицииШаблонаВыполнить = Найти(СтрокаОбработчика, "ВЫПОЛНИТЬ");
...
//////////////////////////////////////////////////////////////////////////////
// ФУНКЦИИ ФОРМИРОВАНИЯ УНИКАЛЬНОГО ИМЕНИ ОБРАБОТЧИКОВ СОБЫТИЙ
...
// Формирует интерфейс глобального обработчика конвертации (уникальное имя процедуры с параметрами соответствующего обработчика).
//
// Параметры:
...
////////////////////////////////////////////////////////////////////////////////
// ПРОЦЕДУРЫ СОЗДАНИЯ ИНТЕРФЕЙСА ВЫЗОВА ОБРАБОТЧИКОВ В ПРАВИЛАХ ОБМЕНА
...
// Параметры:
//  ПКО - Строка таблицы значений    - содержит правило конвертации объекта
//  ПравилаКонвертацииСвойствОбъекта - ТаблицаЗначений - содержит правила конвертации свойств или группы свойств объекта из правила ПКО
//  ПКГС - Строка таблицы значений   - содержит правило конвертации группы свойств
//  
...
Процедура ДобавитьИнтерфейсОбработчикаПКС(Таблица, ПКО, ПКГС = Неопределено, ПКС, ИмяОбработчика) 
...
Функция ОпределитьПоВерсииПлатформыПриемникаПлатформу(ВерсияПлатформы)
	...
	Если Найти(ВерсияПлатформы, "8.") > 0 Тогда
...
Функция НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
	...
	Объект = одНайтиОбъектПоСвойству(СтруктураСвойств.Менеджер, СвойствоПоиска, ЗначениеСвойстваПоиска, 
		НайденныйИлиСозданныйОбъект, , , РежимПоискаОсновногоОбъекта, СтрокаПоиска);
...
Функция ОпределитьПоТипуОбъектаАлгоритмПоследовательностиПолейПоиска(ТипСсылкиСтрокой)
	...
	Попытка
		...
		Для Каждого Элемент Из Правила Цикл
			...
			Если Элемент.Значение.Приемник = ТипСсылкиСтрокой Тогда
				...
				Если Элемент.Значение.ЕстьОбработчикПоследовательностьПолейПоиска = Истина Тогда
					...
					мСоответствиеАлгоритмовПоискаЭлементов.Вставить(ТипСсылкиСтрокой, АлгоритмПоиска);
		...
		мСоответствиеАлгоритмовПоискаЭлементов.Вставить(ТипСсылкиСтрокой, "");
		Возврат "";
...
Функция ОбработатьПоискОбъектаПоСтруктуре(НомерОбъекта, ТипОбъекта, СозданныйОбъект, 
	...
	ЭтоГлобальныйНомер, ПараметрыОбъекта = Неопределено)
...
Функция НайтиОбъектПоСсылке(ТипОбъекта, 
	...
	ИначеЕсли (СвойствоУникальныйИдентификатор <> Неопределено) Тогда
		...
		СсылкаНаОбъект = НайтиИлиСоздатьОбъектПоСвойству(СтруктураСвойств, ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать,
			...
			РежимПоискаОсновногоОбъекта, СвойстваОбъектаМодифицированы, ИзвестнаяСсылкаУникальногоИдентификатора, 
			НппСсылки, НППГлобальнойСсылки, ПараметрыОбъекта);
...
Процедура УстановитьПометкуУдаленияУОбъекта(Объект, ПометкаУдаления, ИмяТипаОбъекта)
	...
	ИначеЕсли ИмяТипаОбъекта = "Документ" И Объект.Проведен Тогда
		
		// для проведенных документов пометку удаления устанавливаем не в режиме загрузки
...
Процедура ДополнитьСтекНеЗаписанныхОбъектов(Нпп, ГНпп, Объект, ИзвестнаяСсылка, ТипОбъекта, ПараметрыОбъекта)
	...
	мГлобальныйСтекНеЗаписанныхОбъектов.Вставить(НомерДляСтека, Новый Структура("Объект, ИзвестнаяСсылка, ТипОбъекта, ПараметрыОбъекта", Объект, ИзвестнаяСсылка, ТипОбъекта, ПараметрыОбъекта));
...
// Выполняет запись объектов в информационную базу из очереди незаписанных объектов (мГлобальныйСтекНеЗаписанныхОбъектов)
// После записи очередь очищается, а записанные объекты запоминаются в кэше для быстрого их поиска при загрузке
//
// Параметры:
//  Нет.
// 
Процедура ПровестиЗаписьНеЗаписанныхОбъектов() Экспорт
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "Ссылка" Тогда
			...
			Ссылка = НайтиОбъектПоСсылке(ТипОбъекта, СвойстваПоиска, СвойстваПоискаНеЗамещать, ОбъектНайден, 
				СозданныйОбъект, НеСоздаватьОбъектЕслиНеНайден, Истина, СвойстваОбъектаМодифицированы, НППГлобальнойСсылки, 
				НппСсылки, ИзвестнаяСсылкаУникальногоИдентификатора, ПараметрыОбъекта);
...
Процедура ВыгрузитьГруппуСвойств(Источник, Приемник, ВходящиеДанные, ИсходящиеДанные, ПКО, ПКГС, УзелКоллекцииСвойств, 
	...
	ВыгружатьГруппуЧерезФайл = ПКГС.ВыгружатьГруппуЧерезФайл 
		И (НепосредственноеЧтениеВИБПриемнике = Ложь);
	...
	Если ВыгружатьГруппуЧерезФайл Тогда
		...
		Исключение
			...
			СтрокаСообщенияОбОшибке = ЗаписатьВПротоколВыполнения(8);
			ЗаписатьИнформациюОбОшибкеОбработчикиКонвертации(1000, ОписаниеОшибки(), ИмяФайлаЗаписей + ": Ошибка при создании временного файла для выгрузки данных");
	...
	Если ПлатформаПриемника = "V8" Тогда
		...
		Если ИмяПредопределенногоЭлемента = Неопределено Тогда
			...
			Если СтруктураСвойств <> Неопределено
				...
				Попытка
					ИмяПредопределенногоИсточник = СтруктураСвойств.Менеджер.ПолучитьИмяПредопределенного(Источник);
				Исключение
	...
	#Если Клиент Тогда
		Если КоличествоОбработанныхОбъектовДляОбновленияСтатуса > 0 
			...
			Попытка
				...
				Состояние("Выгрузка объектов """ + Строка(Источник) + 
					""". Выгружено: " + Строка(мСчетчикВыгруженныхОбъектов) +
					" Объект: " + ИмяМетаданных);
					
			Исключение
				
				Состояние("Выгрузка объектов """ +  
					""". Выгружено: " + Строка(мСчетчикВыгруженныхОбъектов) +
					" Объект: " + ИмяМетаданных);
...
Функция ПолучитьСтрокуОграниченияПоДатеДляЗапроса(Свойства, ИмяТипа, ИмяАлиасаТаблицы, ВыборкаДляОчисткиДанных = Ложь) Экспорт
...
Функция ПолучитьРезультатЗапросаДляВыгрузкиОчисткиДанных(Свойства, ИмяТипа, 
	...
	Если ИмяТипа = "Справочник" 
		...
		Если ИмяТипа = "Справочник" Тогда
			МатаданныеОбъектов = Метаданные.Справочники[Свойства.Имя];
		ИначеЕсли ИмяТипа = "ПланВидовХарактеристик" Тогда
		    МатаданныеОбъектов = Метаданные.ПланыВидовХарактеристик[Свойства.Имя];			
		ИначеЕсли ИмяТипа = "ПланСчетов" Тогда
		    МатаданныеОбъектов = Метаданные.ПланыСчетов[Свойства.Имя];
		ИначеЕсли ИмяТипа = "ПланВидовРасчета" Тогда
		    МатаданныеОбъектов = Метаданные.ПланыВидовРасчета[Свойства.Имя];
		ИначеЕсли ИмяТипа = "РегистрБухгалтерии" Тогда
		    МатаданныеОбъектов = Метаданные.РегистрыБухгалтерии[Свойства.Имя];
		ИначеЕсли ИмяТипа = "ПланОбмена" Тогда
		    МатаданныеОбъектов = Метаданные.ПланыОбмена[Свойства.Имя];
		ИначеЕсли ИмяТипа = "Задача" Тогда
		    МатаданныеОбъектов = Метаданные.Задачи[Свойства.Имя];
		ИначеЕсли ИмяТипа = "БизнесПроцесс" Тогда
		    МатаданныеОбъектов = Метаданные.БизнесПроцессы[Свойства.Имя];			
		КонецЕсли;
		...
		Иначе
			...
			Если ВыгружатьТолькоРазрешенные
				...
				ИмяПервогоРеквизита = ПолучитьИмяПервогоРеквизитаМетаданных(МатаданныеОбъектов);
		...
		Если ВыборкаДляОчисткиДанных
			...
			Если (ИмяТипа = "Справочник"
				...
				Попытка
					НужнаИерархия = МатаданныеОбъектов.Иерархический;					
				Исключение
					НужнаИерархия = Ложь
				КонецПопытки;
...
Функция ПолучитьВыборкуДляВыгрузкиСОграничениями(Правило, ВыборкаДляПодстановкиВПКО = Неопределено, Свойства = Неопределено)
	...
	Иначе
		...
		Иначе
			...
			ПоляВыборки = "Объект.Ссылка КАК Ссылка";
			
		КонецЕсли;
		
		ИтоговоеОграничениеПоДате = ПолучитьСтрокуОграниченияПоДатеДляЗапроса(Свойства, Свойства.ИмяТипа, "Объект", Ложь);
		
		ПостроительОтчета.Текст = "ВЫБРАТЬ " + СтрокаРазрешения + " " + ПоляВыборки + " ИЗ " + ИмяМетаданных + " КАК Объект 
		|
		|" + ИтоговоеОграничениеПоДате + "
		|
		|{ГДЕ Объект.Ссылка.* КАК " + СтрЗаменить(ИмяМетаданных, ".", "_") + "}";
...
Процедура ОткрытьФайлЗагрузки(ТолькоПрочитатьШапку=Ложь, ДанныеФайлаОбмена = "") Экспорт
	...
	// Архивные файлы будем идентифицировать по расширению ".zip"
	Если Найти(ИмяФайлаОбмена, ".zip") > 0 Тогда
	...
	Для Каждого СтрокаПолей Из ПКС Цикл
		...
		Если СтрокаПолей.ЭтоГруппа ТОгда
			...
			Если СтрокаПолей.ВидПриемника = "ТабличнаяЧасть" 
				...
				ВнутреняяСтруктура = СтруктураДанных[ИмяСтруктурыПриемника];
				
				Если ВнутреняяСтруктура = Неопределено Тогда
					ВнутреняяСтруктура = Новый Соответствие();
				КонецЕсли;
				
				СтруктураДанных[ИмяСтруктурыПриемника] = ВнутреняяСтруктура;
				
			Иначе
				
				ВнутреняяСтруктура = СтруктураДанных;	
				
			КонецЕсли;
			
			ЗаполнитьСвойстваДляПоиска(ВнутреняяСтруктура, СтрокаПолей.ПравилаГруппы);
...
Процедура УдалитьЛишниеЭлементыИзСоотвествия(СтруктураДанных)
	
	Для Каждого Элемент Из СтруктураДанных Цикл
		
		Если ТипЗнч(Элемент.Значение) = одТипСоответствие Тогда
			
			УдалитьЛишниеЭлементыИзСоотвествия(Элемент.Значение);
...
Процедура ЗаполнитьИнформациюПоТипамДанныхПриемника(СтруктураДанных, Правила)
	...
	УдалитьЛишниеЭлементыИзСоотвествия(СтруктураДанных);	
...
Процедура ЗагрузитьОдинТипДанных(ПравилаОбмена, СоотвествиеТипа, ИмяЛокЭлемента)
	...
	ИначеЕсли ПравилаОбмена.ТипУзла = одТипУзлаXML_НачалоЭлемента Тогда
		...
		СоотвествиеТипа.Вставить(ИмяУзла, НовоеСоотвествие);
		
		ЗагрузитьОдинТипДанных(ПравилаОбмена, НовоеСоотвествие, ПравилаОбмена.ЛокальноеИмя);			
		ПравилаОбмена.Прочитать();
		
	Иначе
		СоотвествиеТипа.Вставить(ИмяУзла, Тип(ПравилаОбмена.Значение));
		ПравилаОбмена.Прочитать();
	КонецЕсли;	
	
	ЗагрузитьСоответствиеТиповДляОдногоТипа(ПравилаОбмена, СоотвествиеТипа);
	
КонецПроцедуры

Процедура ЗагрузитьСоответствиеТиповДляОдногоТипа(ПравилаОбмена, СоотвествиеТипа)
...
Процедура ЗагрузитьИнформациюОТипахДанных()
	...
	Пока ФайлОбмена.Прочитать() Цикл
		...
		Если ИмяУзла = "ТипДанных" Тогда
			...
			СоотвествиеТипа = Новый Соответствие;
			мСоответствиеТиповДанныхДляЗагрузки.Вставить(Тип(ИмяТипа), СоотвествиеТипа);

			ЗагрузитьСоответствиеТиповДляОдногоТипа(ФайлОбмена, СоотвествиеТипа);	
...
Функция ПолучитьИзТекстаЗначениеОбработчика(ПравилаОбмена)
	...
	Если Найти(ТекстОбработчика, Символы.ПС) = 0 Тогда
...
Процедура ЗагрузитьПравилаОбмена(Источник="", ТипИсточника="XMLФайл") Экспорт
	...
	Пока ПравилаОбмена.Прочитать() Цикл
		...
		ИначеЕсли ИмяУзла = "ПередЗагрузкойОбъекта" Тогда
			Конвертация.Вставить("ПередЗагрузкойОбъекта", ПолучитьИзТекстаЗначениеОбработчика(ПравилаОбмена));
			ЕстьГлобальныйОбработчикПередЗагрузкойОбъекта = Не ПустаяСтрока(Конвертация.ПередЗагрузкойОбъекта);
			одЗаписатьЭлемент(ЗаписьXML, ИмяУзла, Конвертация.ПередЗагрузкойОбъекта);

		ИначеЕсли ИмяУзла = "ПослеЗагрузкиОбъекта" Тогда
			Конвертация.Вставить("ПослеЗагрузкиОбъекта", ПолучитьИзТекстаЗначениеОбработчика(ПравилаОбмена));
			ЕстьГлобальныйОбработчикПослеЗагрузкиОбъекта = Не ПустаяСтрока(Конвертация.ПослеЗагрузкиОбъекта);
		...
		ИначеЕсли ИмяУзла = "ПередЗагрузкойДанных" Тогда
			Конвертация.ПередЗагрузкойДанных = ПолучитьИзТекстаЗначениеОбработчика(ПравилаОбмена);
			одЗаписатьЭлемент(ЗаписьXML, ИмяУзла, Конвертация.ПередЗагрузкойДанных);
			
		ИначеЕсли ИмяУзла = "ПослеЗагрузкиДанных" Тогда
			Конвертация.ПослеЗагрузкиДанных = ПолучитьИзТекстаЗначениеОбработчика(ПравилаОбмена);
	...
	Если Конвертация.Свойство("ПослеЗагрузкиПравилОбмена", ТекстСобытияПослеЗагрузкиПравилОбмена)
		...
		//Отладка данного обработчика не поддерживается!
		Попытка
		...
		Исключение
			...
			Сообщить("Обработчик: ""ПослеЗагрузкиПравилОбмена"": " + ОписаниеОшибки(), СтатусСообщения.Важное); 
...
Процедура ОткрытьДанныеПротоколовОбменаПриНеобходимости()
	...
	Если НЕ ОткрыватьПротоколыОбменаПослеВыполненияОпераций Тогда
		Возврат;
	КонецЕсли;
				
	Если Не ПустаяСтрока(ИмяФайлаПротоколаОбмена) Тогда
		ЗапуститьПриложение("explorer " + ИмяФайлаПротоколаОбмена);
	КонецЕсли;
	
	Если НепосредственноеЧтениеВИБПриемнике Тогда
	
		ИмяПротоколаЗагрузки = ПолучитьИмяПротоколаДляВторойИнформационнойБазыComСоединения();
		
		Если Не ПустаяСтрока(ИмяПротоколаЗагрузки) Тогда
			ЗапуститьПриложение("explorer " + ИмяПротоколаЗагрузки);
		КонецЕсли;
...
Процедура ОткрытьФормуНастройкиОтладкиОбработчиков(ОбъектВладелец, ФормаВладелец, ОбработчикиСобытийИзФайлаПравил)
	
	ФормаНастройкиОтладкиОбработчиков = ПолучитьФорму("ФормаНастройкиОтладкиОбработчиков", ФормаВладелец);
	
	ФормаНастройкиОтладкиОбработчиков.ИмяФайлаВнешнейОбработкиОбработчиковСобытий = ОбъектВладелец.ИмяФайлаВнешнейОбработкиОбработчиковСобытий;
	ФормаНастройкиОтладкиОбработчиков.РежимОтладкиАлгоритмов                      = ОбъектВладелец.РежимОтладкиАлгоритмов;
	ФормаНастройкиОтладкиОбработчиков.ОбработчикиСобытийЧитаемИзФайлаПравилОбмена = ОбработчикиСобытийИзФайлаПравил;
	
	Если ФормаНастройкиОтладкиОбработчиков.ОткрытьМодально() = Истина Тогда
		
		ОбъектВладелец.ИмяФайлаВнешнейОбработкиОбработчиковСобытий = ФормаНастройкиОтладкиОбработчиков.ИмяФайлаВнешнейОбработкиОбработчиковСобытий;
		ОбъектВладелец.РежимОтладкиАлгоритмов                      = ФормаНастройкиОтладкиОбработчиков.РежимОтладкиАлгоритмов; 
		
	КонецЕсли;
	
	//Будем считать, что при открытии формы были прочитаны правила обмена.
	//При этом надо указать обработке (ОбъектВладелец), что нужно перечитать 
	//правила обмена
	Попытка
		//При наличии флага ОбъектВладелец потребует перечитать
		//правила при попытке выгрузки данных
		ОбъектВладелец.мБылиПрочитаныПравилаОбменаПриЗагрузке = Истина;
	Исключение
	КонецПопытки;
	
КонецПроцедуры

Процедура ОткрытьФормуНастройкиОтладкиОбработчиковВыгрузки(ОбъектВладелец, ФормаВладелец) Экспорт
	
	ОткрытьФормуНастройкиОтладкиОбработчиков(ОбъектВладелец, ФормаВладелец, Истина);
	
КонецПроцедуры  

Процедура ОткрытьФормуНастройкиОтладкиОбработчиковВыгрузкиСПравиламиИзМакета(ОбъектВладелец, ФормаВладелец, ИмяМакета) Экспорт
	
	//Получаем уникальное имя файла
	ИмяФайлаПравилОбмена = ПолучитьНовоеУникальноеИмяВременногоФайла("ПравилаОбмена", "xml", "");
	
	МакетПравилОбмена = ОбъектВладелец.ПолучитьМакет(ИмяМакета);
	МакетПравилОбмена.Записать(ИмяФайлаПравилОбмена);
	
	ОткрытьФормуНастройкиОтладкиОбработчиков(ОбъектВладелец, ФормаВладелец, Истина);
	
	//Удаляем временный файл правил обмена
	УдалитьВременныеФайлы(ИмяФайлаПравилОбмена);
	
	ИмяФайлаПравилОбмена = "1";
	
КонецПроцедуры

Процедура ОткрытьФормуНастройкиОтладкиОбработчиковЗагрузки(ОбъектВладелец, ФормаВладелец) Экспорт
	
	ОткрытьФормуНастройкиОтладкиОбработчиков(ОбъектВладелец, ФормаВладелец, Ложь);
...
Процедура СжатьРезультирующийФайлОбмена()
	...
	Попытка
		...
		Архиватор = Новый ЗаписьZipФайла(ИмяФайлаОбмена, ПарольДляСжатияФайлаОбмена, "Файл обмена данными");
...
Функция РаспаковатьZipФайл(ИмяФайлаДляРаспаковки) Экспорт
...
Процедура ИнициализацияСообщений()
	...
	одСообщения.Вставить(75, "Ошибка при выполнении алгоритма после загрузки значений параметров");
	
	одСообщения.Вставить(76, "Ошибка в обработчике события ПослеВыгрузкиОбъектаВФайл");
	
	одСообщения.Вставить(77, "Не указан файл внешней обработки с подключаемыми процедурами обработчиков событий.");
	
	одСообщения.Вставить(78, "Ошибка создания внешней обработки из файла с процедурами обработчиков событий.");
	
	одСообщения.Вставить(79, "Внимание! Рекурсивный вызов алгоритмов! 
	                         |Код алгоритмов не может быть интегрирован в обработчик! 
	...
	одСообщения.Вставить(1000, "Ошибка при создании временного файла выгрузки данных");
...
Процедура ИнициализацияРеквизитовИМодульныхПеременных()
	...
	мСоотвествиеПустыхЗначенийТипов = Новый Соответствие;
	мСоотвествиеОписаниеТипов = Новый Соответствие;
	...
	ИначеЕсли ФлагРежимОтладкиОбработчиков Тогда
		...
		Попытка
			...
			#Иначе
				...
				// На сервере не можем создать внешнюю обработку с модулем отладки
				ВызватьИсключение "Отладка обработчиков не поддерживается при серверном варианте использования обработки ""УниверсальныйОбменДаннымиXML""";
