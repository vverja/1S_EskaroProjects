...
Перем мЧислоЗнаковПослеЗапятойПриОграничениийПоЧислу;
...
// функция определяет можно ли наложить огранияения на все допустимые типы колонки
...
Функция ВернутьСписокПолейОбъекта(СтрокаТипаОбъекта, СтрокаИмениОбъекта, МетаданныеОбъекта) Экспорт
	...
	Если ВРЕГ(СтрокаТипаОбъекта) = "СПРАВОЧНИКИ" 
		...
		// могут быть еще и табличтые части
...
Функция ДобавитьНовыйОбъектПоискаВДерево(Дерево, ИмяОбъектаПоиска, ИмяПользовательскоеОписаниеТаблицыПоиска) Экспорт
	...
	СинономТаблицы = "";
	ТаблицаОбъекта = ПолучитьПолноеИмяОбъекта(ИмяОбъектаПоиска, СинономТаблицы);
	...
	Если ПустаяСтрока(ИмяПользовательскоеОписаниеТаблицыПоиска) Тогда
		СтрокаПредка.ОписаниеТаблицыПоиска = СинономТаблицы;
	Иначе
...
Функция ДобавитьНовуюОбластьПоискаВДерево(СтрокаПредка, ИмяТаблицаПоиска, ИмяПолеПоиска, ИмяПолеСвязки, ИмяПользовательскоеОписаниеТаблицыПоиска) Экспорт
	...
	СинономТаблицы = "";
	ТаблицаОбъекта = ПолучитьПолноеИмяОбъекта(ИмяТаблицаПоиска, СинономТаблицы);
	...
	Если СтрокаПредкаДанных = Неопределено Тогда
		...
		Если ПустаяСтрока(ИмяПользовательскоеОписаниеТаблицыПоиска) Тогда
			СтрокаПредкаДанных.ОписаниеТаблицыПоиска = СинономТаблицы;
		Иначе
...
//		РезультатаЗагрузки - булево. Удачно или нет произошла загрузка даных макета
...
// функция возвращает строку огран
...
Функция СформироватьНачалоЗапроса(Знач ТаблицаПоиска, Знач ИмяПоляВозврата, Знач НужноРазрешенные, Знач КоличествоСтрокДляВыбора, 
	...
	СтракаВыбораРазличных = ?(НЕ ЗначениеЗаполнено(ИмяПоляПоиска) ИЛИ НЕ РасширенныйПоиск, " РАЗЛИЧНЫЕ ", "");
	
	НачалоЗапроса = ?(НужноРазрешенные, "ВЫБРАТЬ " + СтракаВыбораРазличных + " РАЗРЕШЕННЫЕ  ", 
			Символы.ПС + " ОБЪЕДИНИТЬ " + Символы.ПС + " ВЫБРАТЬ " + СтракаВыбораРазличных );
...
// функция находит все ссылки перечисления удовлетвряющие условиям ограничений по строке
...
// Функция формирует ограничение по Чсиловому полю
Функция СформироватьОграничениеПоЧисловомуПолю(ИмяПоля, СоставнойТипПоля = Ложь, СтрокаДополнительногоОграничения = "")
	...
	Если мЧислоДляОграничения <> Неопределено Тогда
		...
		СтрокаПриведения = " КАК Число(32, " + Строка(мЧислоЗнаковПослеЗапятойПриОграничениийПоЧислу) + ")";
...
Функция СформироватьОграничениеПоТипу(ИмяПоляПоиска, УникальноеИмяТаблицыЗапроса, ТипПоля, ФормироватьСложноеОграничениеПоПолю, 
	...
	Если НомерОграничения = 1 Тогда
		...
		// код у справочников не имет тип ни сторка ни число
	...
	ИначеЕсли НомерОграничения = 3 Тогда
		...
		// надо нужный справочник приджойнить
		...
		Если МетаданныеСправочника.ОсновноеПредставление = Метаданные.СвойстваОбъектов.ОсновноеПредставлениеСправочника.ВВидеКода Тогда
			...
			Иначе
				...
				// ограничение накладываем на приджойненный справочник
...
Функция ВернутьСтрокуОграниченийДляПоля(ИмяПоляПоиска, УникальноеИмяТаблицыЗапроса, МассивТиповПоля, Построитель, 
	...
	СложноеОграничениеПоПолю = МножествоТиповУПоля ИЛИ (ОбщегоНазначения.ВернутьИндексВМассиве(МассивТиповПоля, мТипЧисло) >= 0);
...
//Функция формирует строку что бы постритель отчета мог понимать как ограничивать по ней данные
...
// Процедура собирает части запроса в единиый запрос
...
// функция возвращает список ссылок для простого врианта поиска данных
Функция ПолучитьСписокСсылокДляПростогоПоиска(СтрокаДереваПредка, НакладыватьОграниченияПоСтроке, КоличествоСтрокДляВыбора, 
	...
	// простой случай посторения запроса
...
Функция ПолучитьТекущиеСсылкиОбъектов(СтрокаДереваПредка, ИмяОсновнойТаблицыПоискаДляОграничений = Неопределено, 
	...
	// если нужно накладывать ограничение по строке и количество полей по котокрым нужно ограничивать не 1 и нужно получать
...
// процедура добавляет конечные стрки в дерево результатов
...
процедура СформироватьГлобальныеПараметрыДляЗапроса(СтруктураПараметров, Знач ФормироватьОграниченияТолькоПоСтроке = Ложь) Экспорт
	...
	СтрокаДляПоискаВЗапросе = ОбщегоНазначения.СформироватьСтрокуДляПоискаВЗапросе(СтруктураПараметров.СтрокаПоиска);
	...
	Если мЧислоДляОграничения <> Неопределено Тогда
		...
		мЧислоЗнаковПослеЗапятойПриОграничениийПоЧислу = 0;
		...
		Пока ВременноеЧисло <> Цел(ВременноеЧисло) Цикл
			...
			мЧислоЗнаковПослеЗапятойПриОграничениийПоЧислу = мЧислоЗнаковПослеЗапятойПриОграничениийПоЧислу + 1;
		КонецЦикла;
...
// Функция создает узел для базового элемента результирующено дерева
...
// функция возвращает количество таблиц по коотрым есть ограничение
...
Функция ПолучитьДанныеПоПараметрамПоиска(СтруктураПараметров) Экспорт
	...
	Для Каждого СтрокаИзДерева из СтруктураПараметров.ДеревоПоиска.Строки Цикл
		...
		// если ищем по принципу удовлетворяния всем уловиям и всписке нет ссылок - то все, дальше искать нет смысла
...
Функция ИнициализироватьПоискОбъектовПоТипу(Тип, ДеревоНастроекПоиска, СтрокаДереваПоискаПоТекущемуТипу, 
	...
	// восстановим настроийки построителей
...
// поцедура формирует список найденных ссылок на объекты по результирующей стурктуре
Функция СформироватьСписокНайденныхОбъектов(ИтоговыйСписокНайденныхCсылок) Экспорт
	
	СписокСсылок = Новый СписокЗначений;
	СписокСсылок.ЗагрузитьЗначения(ИтоговыйСписокНайденныхCсылок.Строки.ВыгрузитьКолонку("Значение"));
...
// установка общих апарметров поиска по настройке
