...
Функция СформироватьДокументы() Экспорт
	...
	// В табличную часть запишем при удачном завершении
	...
	Если НЕ РегламентноеФормированиеДокументов.УдалитьУчтенныеДокументы(ЭкземплярРегламента, ТаблицаУчтенныеДокументы, Истина, ТекстОшибки) Тогда
		ОтменитьТранзакцию();
		ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, , Заголовок);
		Возврат Ложь;
	КонецЕсли;
	
	Если НЕ РегламентноеФормированиеДокументов.ПровестиСформированныеДокументы(ЭкземплярРегламента, МассивСформированныеДокументы, Истина, ТекстОшибки) Тогда
		ОтменитьТранзакцию();
		ОбщегоНазначения.СообщитьОбОшибке(ТекстОшибки, , Заголовок);
		Возврат Ложь;
	КонецЕсли;
