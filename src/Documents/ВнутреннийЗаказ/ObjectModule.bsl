...
Функция ПечатьСчетаЗаказа(Тип)
	...
	|ИЗ
	|	Документ.ВнутреннийЗаказ.ВозвратнаяТара КАК ВнутреннийЗаказ
	|
	|ГДЕ
	|	ВнутреннийЗаказ.Ссылка = &ТекущийДокумент
	|
	|УПОРЯДОЧИТЬ ПО
	|	НомерСтрокиТЧ
	|";
...
Процедура ОбработкаЗаполнения(Основание)
	...
	ИначеЕсли ТипЗнч(Основание) = Тип("ДокументСсылка.ЗаказПокупателя") Тогда
		...
		Для каждого Стр Из Основание.Товары Цикл
			Если Стр.Размещение = Заказчик Тогда
				Продолжить;
			КонецЕсли;
		...
		Для каждого Стр Из Основание.ВозвратнаяТара Цикл
			Если Стр.Размещение = Заказчик Тогда
				Продолжить;
			КонецЕсли;
		    НовСтр = ВозвратнаяТара.Добавить();
			ЗаполнитьЗначенияСвойств(НовСтр,Стр);
		КонецЦикла;

	//Смартис Лиманчук окончание 25.04.2012
	КонецЕсли;
