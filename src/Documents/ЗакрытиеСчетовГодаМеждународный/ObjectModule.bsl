...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |ИЗ
	               |	РегистрБухгалтерии.Международный.Остатки(&Дата,  Счет В ИЕРАРХИИ (&ЗакрываемыеСчета) И Счет <>&Счет999, , Организация = &Организация ) КАК МеждународныйОстатки";
	...
	Запрос.УстановитьПараметр("Дата", Новый Граница(КонецГода(ДобавитьМесяц(Дата,-12)), ВидГраницы.Включая));
 	...
 	МассивСчетов.Добавить(ПланыСчетов.Международный.Выручка);
 	МассивСчетов.Добавить(ПланыСчетов.Международный.РасходыНаПродажу);
 	МассивСчетов.Добавить(ПланыСчетов.Международный.ПрочиеДоходыИРасходы);
 	МассивСчетов.Добавить(ПланыСчетов.Международный.ПрибылиИУбытки);
 	Запрос.УстановитьПараметр("ЗакрываемыеСчета", МассивСчетов);
 	Запрос.УстановитьПараметр("Счет999", ПланыСчетов.Международный.СуммарныеДоходыИРасходы);
