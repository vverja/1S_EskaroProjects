...
Функция ПечатьДокумента()
	...
	Если НЕ ЗначениеЗаполнено(Шапка.ВнутреннийЗаказ) Тогда
	...
	ОбластьМакета.Параметры.ТекстЗаголовка      = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка,НСтр("ru='Корректировка внутреннего заказа';uk='Корегування внутрішнього замовлення'",КодЯзыкаПечать), КодЯзыкаПечать);
	...
	ОбластьМакета.Параметры.Заполнить(Шапка);
	НазваниеДокумента = НСтр("ru='Внутренний заказ';uk='Внутрішнє замовлення'",КодЯзыкаПечать);
	ОбластьМакета.Параметры.ПредставлениеЗаказа = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка.ВнутреннийЗаказ, НазваниеДокумента, КодЯзыкаПечать);
	ТабДокумент.Вывести(ОбластьМакета);
	...
	|	1 КАК НомерТЧ
	|ИЗ
	|	Документ.КорректировкаВнутреннегоЗаказа.Товары КАК ТабЧасть
	|ГДЕ
	...
	|	NULL КАК Серия,
	|	ТабЧасть.Номенклатура.Артикул,
	|	ТабЧасть.Номенклатура.Код,
	|	ТабЧасть.Номенклатура.НаименованиеПолное,
	|	ТабЧасть.Количество,
	|	ТабЧасть.Номенклатура.ЕдиницаХраненияОстатков,
	|	2
	|ИЗ
	|	Документ.КорректировкаВнутреннегоЗаказа.ВозвратнаяТара КАК ТабЧасть
	|ГДЕ
	|	ТабЧасть.Ссылка = &ТекущийДокумент
	|
	|УПОРЯДОЧИТЬ ПО
	...
	Для Сч = 1 По 2 Цикл
		...
		Пока ВыборкаСтрокТовары.Следующий() Цикл
			...
			Если Сч = 1 И ВыборкаСтрокТовары.Количество < 0 
				ИЛИ Сч = 2 И ВыборкаСтрокТовары.Количество > 0 Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ДвиженияПоРегистрамУпр(РежимПроведения, СтруктураШапкиДокумента, 
	...
	НаборДвижений.КонтрольОстатков(ЭтотОбъект, Параметры, Отказ, Заголовок, РежимПроведения);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Организация 	= ВнутреннийЗаказ.Организация;
