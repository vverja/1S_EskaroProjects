...
Процедура Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	Если ИмяМакета = "Опрос" Тогда
		...
		УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер, ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,Метаданные().Синоним), Ссылка);
...
Функция ПолучитьСтруктуруПечатныхФорм() Экспорт
	...
	Возврат Новый Структура("Опрос,HTML","Опрос","Просмотр HTML");
...
Функция ПроверитьОбязательноЗаполняемыеОтветы(ПоВопросу = Неопределено, НеПроверкаНаУсловноНеЗапольнять = Истина) Экспорт
	...
	Если ПоВопросу <> Неопределено Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "//ОТБОР_ПО_ВОПРОСУ", "И ТиповыеАнкетыВопросыАнкеты.Вопрос = &ПоВопросу");
	КонецЕсли;
...
Функция УдовлетворяетУсловию(Вопрос, Параметр, Обязательный = Неопределено, НеПроверкаНаУсловноНеЗапольнять = Ложь)
	...
	Если Обязательный = Перечисления.ОбязательностьЗаполненияОтветаНаВопрос.УсловноНеЗаполнять Тогда
		...
		Если Вопрос.ТипВопроса = Перечисления.ТипВопросаАнкеты.Табличный Тогда
			...
			Для каждого СтрокаСостовногоОтвета из СтрокиПоиска Цикл
				Если ЗначениеЗаполнено(СтрокаСостовногоОтвета.ТиповойОтвет) Тогда
					Возврат Истина;
				КонецЕсли;
			КонецЦикла;
			Возврат Ложь;
