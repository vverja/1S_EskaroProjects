...
Процедура ПроверитьЗаполнениеСтрокиИспользованиеОтпусков(ВыборкаПоСтрокамДокумента, Отказ, Заголовок)
	...
	СтрокаНачалаСообщенияОбОшибке = "Для сотрудника """+ СокрЛП(ВыборкаПоСтрокамДокумента.ФизЛицо) +
	""" табл. части ""Использование"": ";

	// ДатаНачала
	ЕстьРабочийПериодНачало = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.РабочийПериодНачало);
	Если Не ЕстьРабочийПериодНачало Тогда
		ОбщегоНазначения.СообщитьОбОшибке(СтрокаНачалаСообщенияОбОшибке + "не указано начало рабочего периода!", Отказ, Заголовок);
	КонецЕсли;

	// ДатаОкончания
	ЕстьРабочийПериодОкончание = ЗначениеЗаполнено(ВыборкаПоСтрокамДокумента.РабочийПериодОкончание);
	Если Не ЕстьРабочийПериодОкончание Тогда
...
Процедура ДобавитьСтрокуПоИспользованиюОтпусков(ВыборкаПоИспользованию) 
	
	ИмяРегистра = "ИспользованиеЕжегодныхОтпусковРаботников";
	
	Движение = Движения[ИмяРегистра].Добавить();
		
	// Свойства
	Движение.Период                 = ВыборкаПоИспользованию.ДатаНачала;

	// Измерения
	Движение.Сотрудник				= ВыборкаПоИспользованию.Сотрудник;
	Движение.Организация			= ВыборкаПоИспользованию.Организация;
	Движение.ВидОтпуска				= ВыборкапоИспользованию.ВидОтпуска;
	//иДвижение.ФизЛицо				= ВыборкаПоИспользованию.ФизЛицо;
...
Процедура ДвиженияПоИспользованиюОтпусков(Заголовок, Отказ, Режим)
	Если ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.Отпуск Тогда
		...
		КонецЦикла;	
	Иначе
		Для Каждого Строка ИЗ ИспользованиеЕжегодногоОтпуска Цикл
			Движение = Движения.ИспользованиеЕжегодныхОтпусковРаботников.Добавить();
			Движение.Период = Дата;
			Движение.ВидОтпуска = Строка.ВидОтпуска;
			Движение.Сотрудник = Строка.Сотрудник;
			Движение.Регистратор = Ссылка;
			Движение.ИспользованоОсновного = Строка.ИспользоватьДней;
			Движение.Организация = Организация;
			Движение.РабочийПериодНачало = Строка.РабочийПериодНачало;
			Движение.РабочийПериодОкончание = Строка.РабочийПериодОкончание;
			Движение.ВидОперации = ВидОперации;
			
			//Если ВидОперации = Перечисления.ВидыОперацийОтпускаОрганизаций.КорректировкаОтпуска Тогда
			//	Движение.ВидДвижения = ВидДвиженияНакопления.Приход;
			//Иначе
			//	Движение.ВидДвижения = ВидДвиженияНакопления.Расход;
			//КонецЕсли;
...
//			ВыборкаПоИспользованию = РезультатЗапросаПоИспользованию.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
//				Если НЕ Отказ Тогда
//					ДобавитьСтрокуПоИспользованиюОтпусков(ВыборкаПоИспользованию);
//				КонецЕсли;
