...
Функция ПечатьПечатьПереоценкиТоваровПринятых()
	...
	|	ПереоценкаТоваровПринятыхНаКомиссию.Ссылка = &ТекущийДокумент
	|УПОРЯДОЧИТЬ ПО
	|	ПереоценкаТоваровПринятыхНаКомиссию.НомерСтроки
	|";
	...
	ОбластьМакета.Параметры.ТекстЗаголовка = ОбщегоНазначения.СформироватьЗаголовокДокумента(Шапка, НСтр("ru='Переоценка товаров принятых на комиссию';uk='Зміна цін товарів прийнятих на комісію'",КодЯзыкаПечать),КодЯзыкаПечать);
	...
	// Выводим дополнитльеные колонки
	...
	Для каждого ВыборкаСтрокТовары из ЗапросТовары Цикл 
		...
		ОбластьСуммыСтарая.Параметры.Заполнить(ВыборкаСтрокТовары);
		ТабДокумент.Присоединить(ОбластьСуммыСтарая);
		ОбластьСуммыНовая.Параметры.Заполнить(ВыборкаСтрокТовары);
		ТабДокумент.Присоединить(ОбластьСуммыНовая);
	...
	ОбластьСуммыСтарая  = Макет.ПолучитьОбласть("Итого|Старая");
	ОбластьСуммыНовая	= Макет.ПолучитьОбласть("Итого|Новая");
	...
	ОбластьСуммыСтарая.Параметры.ВсегоПоСтарым = ОбщегоНазначения.ФорматСумм(Товары.Итог("СуммаСтарая"));
	ТабДокумент.Присоединить(ОбластьСуммыСтарая);
	ОбластьСуммыНовая.Параметры.ВсегоПоНовым   = ОбщегоНазначения.ФорматСумм(Товары.Итог("Сумма"));
	ТабДокумент.Присоединить(ОбластьСуммыНовая);
...
Процедура ПодготовитьТаблицуТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента)
	...
	КурсВалютыРегламентированногоУчета = МодульВалютногоУчета.ПолучитьКурсВалюты(мВалютаРегламентированногоУчета, Дата);

	// Надо заполнить новые колонки.
	Для каждого СтрокаТаблицы Из ТаблицаТоваров Цикл

		// Документ всегда включает сумму с НДС. В партиях БУ эта сумма должна 
		// быть разделена на себестоимость и НДС
		СуммаСНДС = СтрокаТаблицы.Сумма;

		// Суммы пересчитаем в валюту регалментированного учета
		Если СтруктураШапкиДокумента.ВалютаДокумента = мВалютаРегламентированногоУчета Тогда
			СуммаСНДСРег = СуммаСНДС;
		Иначе
			СуммаСНДСРег = МодульВалютногоУчета.ПересчитатьИзВалютыВВалюту(СуммаСНДС, СтруктураШапкиДокумента.ВалютаДокумента,
											 мВалютаРегламентированногоУчета,
											 СтруктураШапкиДокумента.КурсДокумента, КурсВалютыРегламентированногоУчета.Курс,
											 СтруктураШапкиДокумента.КратностьДокумента,КурсВалютыРегламентированногоУчета.Кратность);
		КонецЕсли;
		
		СтрокаТаблицы.ПроводкаСумма = СуммаСНДСРег - Ценообразование.РассчитатьСуммуНДС(СуммаСНДСРег, Истина, Истина, Ценообразование.ПолучитьСтавкуНДС(СтрокаТаблицы.СтавкаНДС))
		
	КонецЦикла;
																
	ПогрешностиОкругления = Новый Структура;
	
	НалоговыйУчет.ДобавитьКолонкиТоваровРегл(ТаблицаТоваров, СтруктураШапкиДокумента, ПогрешностиОкругления, Истина, Ложь, "ИзменениеСуммы");
