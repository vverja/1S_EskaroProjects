...
//Коновалова 30102013 +
Процедура ЗаполнитьТЧПоОсновномуПоставщику() Экспорт
	
	Для каждого строкаТЧ из ЭтотОбъект.СоставПлана Цикл
		Если ТипЗнч(строкаТЧ.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			строкаТЧ.Контрагент = строкаТЧ.Номенклатура.ОсновнойПоставщик;
			Если строкаТЧ.Контрагент <> Справочники.Номенклатура.ПустаяСсылка() Тогда
				строкаТЧ.Договор = строкаТЧ.Контрагент.ОсновнойДоговорКонтрагента;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;	
