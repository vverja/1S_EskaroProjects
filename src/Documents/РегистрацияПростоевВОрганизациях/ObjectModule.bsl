...
Функция ПечатьСредняя() Экспорт
	...
	|ИЗ
	|	Документ.РегистрацияПростоевВОрганизациях.РасчетСреднего КАК ОплатаПоСреднемуЗаработку
	|ГДЕ
	...
	Иначе
		ОбластьШапка2.Параметры.СредняяТекст = "Среднечасовая (по календарным):";
	КонецЕсли;
...
Функция Печать(ИмяМакета, КоличествоЭкземпляров = 1, НаПринтер = Ложь) Экспорт
	...
	УниверсальныеМеханизмы.НапечататьДокумент(ТабДокумент, КоличествоЭкземпляров, НаПринтер,ОбщегоНазначения.СформироватьЗаголовокДокумента(ЭтотОбъект,"Регистрация простоев "));
...
Функция ПолучитьРеквизитыНачисления() Экспорт
	
	Запрос = Новый Запрос;
	
	Запрос.УстановитьПараметр("парамСотрудник",Сотрудник);
	Запрос.УстановитьПараметр("ДатаАктуальности", ДатаНачала);
	Запрос.УстановитьПараметр("ВидРасчета", ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.Простой);
	...
	|		*	ВЫБОР	КОГДА ОсновноеНачисление.Валюта1 = &РегламентВалюта ТОГДА 1
	|				ИНАЧЕ ЕСТЬNULL(Валюты.Курс, 0) * ЕСТЬNULL(Валюты.Кратность, 0) 
	|			КОНЕЦ 						КАК ОкладТариф,
	|	ОсновноеНачисление.ВидРасчета.СпособРасчета КАК СпособРасчета
	|
	|ИЗ	РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаАктуальности, Сотрудник = &парамСотрудник) КАК Работники
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПлановыеНачисленияРаботниковОрганизаций.СрезПоследних(&ДатаАктуальности, Сотрудник = &парамСотрудник И ВидРасчетаИзмерение = Неопределено ) КАК ОсновноеНачисление
	|ПО		ИСТИНА
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РазмерыЗаконодательноУстановленныхВыплат.СрезПоследних(&ДатаАктуальности, ВидРасчета = &ВидРасчета) КАК РазмерыЗаконодательноУстановленныхВыплат
	|ПО		ИСТИНА
	|
	...
	Если ВидПростоя = Перечисления.ВидыПростояРаботника.ПоНезависящимПричинам Тогда
		...
		Если СпособРегистрацииВремени = Перечисления.СпособыРегистрацииВремени.РегистрацияДляЧастиСмены Тогда
			ВидРасчета = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПочасовойПростойПоСредней;
		Иначе
	...
	Иначе
		...
		Если ДанныеНаДатуНачисления.Следующий() Тогда
			...
			Если СпособРегистрацииВремени = Перечисления.СпособыРегистрацииВремени.РегистрацияДляЧастиСмены Тогда
				ВидРасчетаКОплате = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.ПочасовойПростой;
			Иначе
				ВидРасчетаКОплате = ПланыВидовРасчета.ОсновныеНачисленияОрганизаций.Простой;
			КонецЕсли;
...
Функция		ПолучитьДанныеОбОтработанномВремени(ОтработаноЗа2Мес, ОтработаноЗа4Мес, ОсновнойВидРасчета)
	...
	ПараметрыПериода = Новый Структура("ГрафикРаботы,ОсновнойВидРасчета,ПериодРасчетаСреднегоНачало,ПериодРасчетаСреднегоКонец");
	ПериодПолучен = Ложь;
...
Функция АвтозаполнениеРасчетСреднего() Экспорт
	...
	Запрос.УстановитьПараметр( "парамКоэф",		Коэф );
...
Процедура АвтозаполнениеНачисления() Экспорт
	...
	|
	|ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.РаботникиОрганизаций.СрезПоследних(&ДатаНачала, Сотрудник = &парамСотрудник ИЛИ Сотрудник.ОсновноеНазначение = &парамСотрудник ) КАК Работники
	|ПО		ИСТИНА
	|
	...
	// каждый цикл это приказ (назначение)
	Пока Выборка.Следующий() Цикл
		...
		Пока ТекущаяДатаНачала <= ТекущаяДатаОкончания Цикл
			...
			// Реквизиты
			ПроведениеРасчетов.ПроставитьДанныеСтроки(Сотрудник, Выборка.Назначение, НоваяСтрока.ДатаНачала, ПериодРегистрации, ВидРасчета, НоваяСтрока);
			Если ВидПростоя = Перечисления.ВидыПростояРаботника.ПоНезависящимПричинам Тогда
...
Процедура ПроверитьЗаполнениеШапки(ВыборкаПоШапкеДокумента, Отказ)
	...
	Если НЕ ЗначениеЗаполнено(ВыборкаПоШапкеДокумента.ВидПростоя) Тогда
		ОбщегоНазначения.СообщитьОбОшибке("Не указан вид простоя!", Отказ);
	КонецЕсли;
