...
Функция ПечатьДокумента()
	...
	Если ВыборкаПоСтрокам.Следующий() Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ОбластьШапкаТипЦен.Параметры.ТекстСкидка = НСтр("ru='Скидка (наценка)';uk='Знижка'",КодЯзыкаПечать);
			ОбластьШапкаТипЦен.Параметры.ТекстЕд = НСтр("ru='Ед.';uk='Од.'",КодЯзыкаПечать);
			ОбластьШапкаТипЦен.Параметры.ТекстЦена = НСтр("ru='Цена';uk='Ціна'",КодЯзыкаПечать);
	...
	ОбластьПодвалНоменклатура.Параметры.ТекстОтветственный = НСтр("ru='Ответственный';uk='Відповідальний'",КодЯзыкаПечать);
