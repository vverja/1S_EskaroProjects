...
Процедура НапечататьПисьмо() Экспорт
	...
	Если ЗначениеЗаполнено(ЗаявкаКандидата) Тогда
		ТекстВыходногоДокумента = ТекстВыходногоДокумента + СтрокаТаблицы(НСтр("ru='Заявка кандидата:';uk='Заява кандидата:'", КодЯзыкаПечать), ПредставлениеСтрокиДляHTML(Строка(ЗаявкаКандидата)));
	КонецЕсли;
