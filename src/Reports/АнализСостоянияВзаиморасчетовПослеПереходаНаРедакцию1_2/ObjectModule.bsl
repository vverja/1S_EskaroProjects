...
Процедура УстановитьНачальныеНастройки(ДополнительныеПараметры = Неопределено) Экспорт
	...
	|	Взаиморасчеты.СуммаВзаиморасчетовПоСделкеНачальныйОстаток КАК СуммаВзаиморасчетовПоСделкеНачальныйОстаток,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоСделкеКонечныйОстаток КАК СуммаВзаиморасчетовПоСделкеКонечныйОстаток,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоСделкеПриход КАК СуммаВзаиморасчетовПоСделкеПриход,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоСделкеРасход КАК СуммаВзаиморасчетовПоСделкеРасход,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоДокументуНачальныйОстаток КАК СуммаВзаиморасчетовПоДокументуНачальныйОстаток,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоДокументуКонечныйОстаток КАК СуммаВзаиморасчетовПоДокументуКонечныйОстаток,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоДокументуПриход КАК СуммаВзаиморасчетовПоДокументуПриход,
	|	Взаиморасчеты.СуммаВзаиморасчетовПоДокументуРасход КАК СуммаВзаиморасчетовПоДокументуРасход,
	|	Взаиморасчеты.РасхождениеНачальныйОстаток КАК РасхождениеНачальныйОстаток,
	|	Взаиморасчеты.РасхождениеКонечныйОстаток КАК РасхождениеКонечныйОстаток,
	|	Взаиморасчеты.РасхождениеПриход КАК РасхождениеПриход,
	|	Взаиморасчеты.РасхождениеРасход КАК РасхождениеРасход
	|	//ПОЛЯ_СВОЙСТВА
	|	//ПОЛЯ_КАТЕГОРИИ
	|{ВЫБРАТЬ
	|	Организация.*,
	...
	|			ПериодГод}
	|		ИЗ
	|			РегистрНакопления.ВзаиморасчетыСКонтрагентамиПоДокументамРасчетов.ОстаткиИОбороты(&ДатаНач, &ДатаКон, Регистратор {(&Периодичность)}, , {(ДоговорКонтрагента).*, (Контрагент).*, (Организация).*, (ДокументРасчетовСКонтрагентом).*}) КАК ВзаиморасчетыПоДокументамРасчетов) КАК ЗапросПоРегистрам
	|	
	|	СГРУППИРОВАТЬ ПО
	...
	|		ЗапросПоРегистрам.ПериодГод) КАК Взаиморасчеты
	|//СОЕДИНЕНИЯ
	|{ГДЕ
	|	Взаиморасчеты.Регистратор.* КАК Регистратор,
	|	Взаиморасчеты.Период КАК Период,
	...
	|	СУММА(РасхождениеНачальныйОстаток),
	|	СУММА(РасхождениеКонечныйОстаток),
	|	СУММА(РасхождениеПриход),
	|	СУММА(РасхождениеРасход)
	|	//ИТОГИ_СВОЙСТВА
	|	//ИТОГИ_КАТЕГОРИИ
	|ПО
	|	ОБЩИЕ
...
Процедура ТорговыеДокументы_ОчиститьРеквизитСделка()
	...
	СписокДокументов.Добавить("АктОбОказанииПроизводственныхУслуг");
    ...
    СписокДокументов.Добавить("ПолучениеУслугПоПереработке");
    ...
    СписокДокументов.Добавить("РеализацияУслугПоПереработке");
	...
	Для каждого ВидДокумента Из СписокДокументов цикл
		...
		Если ВидДокумента.Значение = "ПолучениеУслугПоПереработке" Тогда
			ИмяРеквизитаСделка = "Заказ";
		КонецЕсли;
...
Процедура ОчиститьИзмерениеСделкаВРегистрахРасчетыПоПриобретениюРеализации()
	...
	масРегистры.Добавить("ПланыДвиженияДенежныхСредств");
