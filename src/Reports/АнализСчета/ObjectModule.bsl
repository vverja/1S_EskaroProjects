...
Процедура СформироватьОтчет(Результат = Неопределено, ДанныеРасшифровки = Неопределено, ВыводВФормуОтчета = Истина, ВнешниеНаборыДанных = Неопределено, ВыводитьПолностью = Истина) Экспорт
	...
	Если ВыводитьПолностью Тогда
		...
		Если РольДоступна("ГруппаА") Тогда
			//НП1 = НастройкаКомпоновкиДанных.ПараметрыДанных.Элементы[1];
			//
			//КП1 = НастройкаКомпоновкиДанных.ПараметрыДанных.Элементы[2];
			//КП2 = НастройкаКомпоновкиДанных.ПараметрыДанных.Элементы[6];
...
Процедура ПередВыводомОтчета(МакетКомпоновки) Экспорт
	...
	МакетГруппировкиНалоговоеНазначениеЗаголовок = СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "НалоговоеНазначение", , "Заголовок");
	МакетГруппировкиНалоговоеНазначениеПодвал    = СтандартныеОтчеты.ПолучитьМакетГруппировкиПоПолюГруппировки(МакетКомпоновки, "НалоговоеНазначение", , "Подвал");
...
Процедура ДоработатьКомпоновщикПередВыводом(ВнешниеНаборыДанных) Экспорт
	...
	Если Не ПоСубсчетамКорСчетов Тогда
		...
		// Если не выбрано ПоКорСубсчетамИСубконто, то выводим 2 верхних уровня - классы счетов и счета 
		ТиповыеОтчеты.ДобавитьОтбор(ГруппаЭлементовОтбора, "СистемныеПоля.УровеньВГруппировке", КоличествоУровнейКорСчет, ВидСравненияКомпоновкиДанных.МеньшеИлиРавно);
...
Функция ПолучитьТекстЗаголовка(ОрганизацияВНачале = Истина) Экспорт 
	...
	ЗаголовокОтчета = НСтр("ru='Анализ счета ';uk='Аналіз рахунку '") + Счет + СтандартныеОтчеты.ПолучитьПредставлениеПериода(ЭтотОбъект);
