	...
	Для Каждого СтрокаПоказатели Из ПоказателиОтчета Цикл
		...
		Иначе
			...
			Иначе
				...
				|ВЫБОР	КОГДА Регистратор ССЫЛКА Документ.ВозвратТоваровОтПокупателя ИЛИ Регистратор ССЫЛКА Документ.ВозвратПереданныхТоваров
				|		ТОГДА " + СтрЗаменить(ОписаниеПоказателя,"#", "Расход") + "
				|		ИНАЧЕ	ВЫБОР	КОГДА " + СтрЗаменить(ОписаниеПоказателя,"#", "Приход") + " < 0
				...
				|						ТОГДА " + СтрЗаменить(ОписаниеПоказателя,"#", "Приход") + "
				|						ИНАЧЕ 0.00
				|				КОНЕЦ + " + СтрЗаменить(ОписаниеПоказателя,"#", "Расход") + "
				|КОНЕЦ КАК " + СтрокаПоказатели.ИмяПоказателя + "Расход,") + "
				|	0 КАК ЛимитПокупателю"+СтрокаПоказатели.ИмяПоказателя+"НачальныйОстаток,
				|	0 КАК ЛимитПоставщика"+СтрокаПоказатели.ИмяПоказателя+"НачальныйОстаток,
				|	0 КАК ЛимитПокупателю"+СтрокаПоказатели.ИмяПоказателя+"КонечныйОстаток,
				|	0 КАК ЛимитПоставщика"+СтрокаПоказатели.ИмяПоказателя+"КонечныйОстаток";
		...
		Если Найти(ОписаниеФильтра, ".Свойство") > 0 Тогда
			...
			Если НайдСтрока <> Неопределено Тогда
				...
				Если    Врег(СтрокаФильтры.ТипФильтра) = "РАВНО" 				ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "ДОРІВНЮЄ"
					...
					ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "В ГРУППЕ ИЗ СПИСКА"   ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "В ГРУПІ ЗІ СПИСКУ" Тогда
				...
				ИначеЕсли Врег(СтрокаФильтры.ТипФильтра) = "НЕ РАВНО"                ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "НЕ ДОРІВНЮЄ"
					  ...
					  ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "НЕ В ГРУППЕ ИЗ СПИСКА"   ИЛИ Врег(СтрокаФильтры.ТипФильтра) = "НЕ В ГРУПІ ЗІ СПИСКУ" Тогда
