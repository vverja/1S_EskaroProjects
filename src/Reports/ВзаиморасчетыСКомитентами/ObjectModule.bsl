...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок, ВысотаЗаголовка, ТолькоЗаголовок = Ложь,  ЕстьОшибки = Ложь) Экспорт
	...
	Запрос.Текст="ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
	             ...
	             |ИЗ
	             |	РегистрНакопления.ПартииТоваровНаСкладах.Обороты(, , , СтатусПартии = &ТоварПринятыйКомиссия) КАК ПартииТоваровКомпанииОбороты
	             |ГДЕ
	             |	ПартииТоваровКомпанииОбороты.КоличествоПриход > 0";
	...
	Для Каждого Строка Из ГруппировкиОтчета цикл
		...
		ЕстьРегистратор = ?(ЕстьРегистратор,Истина,Строка.ИмяГруппировки = "Регистратор");
		ОписаниеПоляЗапроса = "";
	КонецЦикла;
