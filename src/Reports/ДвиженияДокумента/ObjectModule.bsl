...
Процедура СформироватьОтчет(ДокументРезультат) Экспорт
	...
	ТаблицаДвижений.Колонки.Добавить("Порядок", Новый ОписаниеТипов("Число", Новый КвалификаторыЧисла(1)));
	ТаблицаДвижений.Колонки.Добавить("ИмяДляСравнения", Новый ОписаниеТипов("Строка"));
	Для Каждого СтрокаТаблицыДвижений Из ТаблицаДвижений Цикл
		
		СтрокаТаблицыДвижений.Имя = СокрЛП(СтрокаТаблицыДвижений.Имя);
		
		Поз = Найти(СтрокаТаблицыДвижений.Имя, ".");
		ВидРегистра = Лев(СтрокаТаблицыДвижений.Имя, Поз - 1);
		ИмяРегистра = Сред(СтрокаТаблицыДвижений.Имя, Поз + 1);
		Если ВидРегистра = "РегистрСведений" Тогда   
			СтрокаТаблицыДвижений.Порядок = 0;
			СтрокаТаблицыДвижений.ИмяДляСравнения = "Сведений." + ИмяРегистра;
		ИначеЕсли ВидРегистра = "РегистрНакопления" Тогда   
			СтрокаТаблицыДвижений.Порядок = 1;
			СтрокаТаблицыДвижений.ИмяДляСравнения = "Накопления." + ИмяРегистра;
		ИначеЕсли ВидРегистра = "РегистрБухгалтерии" Тогда   
			СтрокаТаблицыДвижений.Порядок = 2;
			СтрокаТаблицыДвижений.ИмяДляСравнения = "Бухгалтерии." + ИмяРегистра;
		ИначеЕсли ВидРегистра = "РегистрРасчета" Тогда   
			СтрокаТаблицыДвижений.Порядок = 3;
			СтрокаТаблицыДвижений.ИмяДляСравнения = "Расчета." + ИмяРегистра;
		КонецЕсли;
		
		СтрокаТаблицыДвижений.Имя = Врег(СтрокаТаблицыДвижений.Имя);
		
	КонецЦикла;
	
	ТаблицаДвижений.Сортировать("Порядок, Имя");
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		ИмяРегистраДляСравнения = ВидРегистра + "." + СокрЛП(СвойстваОбъекта.Имя);
		Если ИмяРегистраДляСравнения <> СтрокаТаблицыДвижений.ИмяДляСравнения Тогда
			Продолжить;	                   
		КонецЕсли; 
		...
		Если (ВидРегистра = "Сведений") 
			...
			Если ВидРегистра+"."+СвойстваОбъекта.Имя = "Сведений.СписанныеТовары"
			 ...
			 ИЛИ ВидРегистра+"."+СвойстваОбъекта.Имя = "Сведений.ДокументыТребующиеДопроведения"
