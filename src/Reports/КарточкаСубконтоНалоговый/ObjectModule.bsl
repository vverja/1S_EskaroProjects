...
Функция ЗаголовокОтчета() Экспорт
	...
	Возврат "Карточка субконто (налоговый)";
...
Процедура ПолучитьТекстЗапроса(Запрос)
	...
	|ИЗ
	|		РегистрБухгалтерии."+ИмяРегистраБухгалтерии+".ДвиженияССубконто(&ДатаНач, &ДатаКон, ) КАК ДвиженияССубконто
	|		
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии."+ИмяРегистраБухгалтерии+".ОстаткиИОбороты(&ДатаНач, &ДатаКон, Запись, , "+ТекстОтборСчетов+", &МассивСубконто, ) КАК ОстаткиИОбороты
...
Процедура ВывестиПроводки(ДокументРезультат, ВыборкаВерхнегоУровня, СтруктураПараметров)
	...
	ВыборкаПроводок = ВыборкаВерхнегоУровня.Выбрать();
