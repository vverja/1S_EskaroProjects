...
Процедура ВыводСубконто(Знач ВыборкаОбороты, Знач ВыборкаОстатки, Знач Инд, СтруктураПараметров, ОтборДляРасшифровки, Знач ВыводКоличества)
	...
	ВыводКоличества = ПоКоличеству И БухгалтерскиеОтчеты.ОпределитьПоСубконтоИПостроителюВестиКоличественныйУчет(Счет, ПостроительОтчета, Измерение);
...
Процедура ВывестиВалюты(Знач ВыборкаОбороты, Знач ВыборкаОстатки, СтруктураПараметров, ОтборДляРасшифровки)
	...
	Пока Истина Цикл
		...
		ИначеЕсли ОборотПрочитан И БухгалтерскиеОтчеты.ЭтоНулевойОборот(ВыборкаОбороты, Ложь) Тогда
		...
		ЗаполнитьИПрисоединитьОбласть(СальдоНачДт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаСальдоНачДт, ВыборкаОстатки, Уровень);
		ЗаполнитьИПрисоединитьОбласть(СальдоНачКт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаСальдоНачКт, ВыборкаОстатки, Уровень);
		...
		// Оборот Дт
		ЗаполнитьИПрисоединитьОбласть(ОборотДт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаОборотДт, ВыборкаОбороты, Уровень);
		
		Если ОборотДтКорСчета Тогда
			
			// Вывод вложенных итогов по кор счетам
			ВывестиКорСчета(ВыборкаОборотыПоКорСчетам, СтруктураПараметров, "ДТ", "Сумма, ВалютнаяСумма", 
				СтруктураПараметров.ОбластьСтрокаВалютаОборотДтКорСчет, ОтборДляРасшифровки, Выборка);
		КонецЕсли;
		
		// Оборот Кт
		ЗаполнитьИПрисоединитьОбласть(ОборотКт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаОборотКт, ВыборкаОбороты, Уровень);
		...
		Если ОборотКтКорСчета Тогда
			...
			ВывестиКорСчета(ВыборкаОборотыПоКорСчетам, СтруктураПараметров, "КТ", "Сумма, ВалютнаяСумма", 
				СтруктураПараметров.ОбластьСтрокаВалютаОборотКтКорСчет, ОтборДляРасшифровки, Выборка);
		КонецЕсли;
				
		// Вывод конечного сальдо
		ЗаполнитьИПрисоединитьОбласть(СальдоКонДт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаСальдоКонДт, ВыборкаОстатки, Уровень);
		ЗаполнитьИПрисоединитьОбласть(СальдоКонКт, СтруктураПараметров.ДокументРезультат, СтруктураПараметров.ОбластьСтрокаВалютаСальдоКонКт, ВыборкаОстатки, Уровень);
...
Процедура НачалоВыбораЗначенияСубконто(Элемент, СтандартнаяОбработка, ТипЗначенияПоля=Неопределено) Экспорт
	...
	Для Инд=0 По ПостроительОтчета.Отбор.Количество()-1 Цикл
		...
		Если СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоМеждународные.Номенклатура.ТипЗначения Тогда
			СписокПараметров.Вставить("Номенклатура", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоМеждународные.Склады.ТипЗначения Тогда
			СписокПараметров.Вставить("Склад", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоМеждународные.Контрагенты.ТипЗначения Тогда
			СписокПараметров.Вставить("Контрагент", ЗначениеОтбора);
		ИначеЕсли СтрокаОтбора.ТипЗначения = ПланыВидовХарактеристик.ВидыСубконтоМеждународные.Договоры.ТипЗначения Тогда
