...
Если РегламентированнаяОтчетность.ИДКонфигурации() = "УПП" Тогда
	...
	НоваяФорма.ОписаниеОтчета     = НСтр("ru='Утверждена приказом ГНАУ N 98 от 28.02.2003 г.';uk='Затверджена наказом ДПАУ N 98 від 28.02.2003 р.'"); 
	НоваяФорма.ДатаНачалоДействия = '20030101';
	НоваяФорма.ДатаКонецДействия  = '20050331';//ОбщегоНазначения.ПустоеЗначениеТипа(Тип("Дата"));
	
	НоваяФорма = мТаблицаФормОтчета.Добавить();
КонецЕсли;

НоваяФорма.ФормаОтчета        = "ФормаОтчета2005Кв2";
НоваяФорма.ОписаниеОтчета     = НСтр("ru='Утверждена приказом ГНАУ N 98 от 28.02.2003 г. (в редакции приказа ГНАУ от 20.04.2005 №145)';uk='Затверджена наказом ДПАУ N 98 від 28.02.2003 р. (у редакції наказу ДПАУ від 20.04.2005 №145)'"); 
НоваяФорма.ДатаНачалоДействия = '20050401';
