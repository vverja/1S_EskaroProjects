  ...
  Функция ТекстЗапросаПоСценарию(СтрокаСценарий,СтруктураПараметров,Номер,ПериодДанных, СтруктураГруппировки)
	...
	Если НЕ СтатьяБаланса.Пустая() Тогда
		...
		ТекстЗапроса="
			...
			|ИЗ
			|	(ВЫБРАТЬ
			|		ОборотыБюджетовДебет.СтатьяОборотов КАК СтатьяОборотов,
			|		ОборотыБюджетовДебет.Период КАК Период,
			|		ОборотыБюджетовДебет.Сценарий КАК Сценарий,
			|		ОборотыБюджетовДебет.Проект КАК Проект,
			|		ОборотыБюджетовДебет.ЦФО КАК ЦФО,
			|		ОборотыБюджетовДебет.Контрагент КАК Контрагент,
			|		ОборотыБюджетовДебет.Номенклатура КАК Номенклатура,
			|		ОборотыБюджетовДебет.Валюта КАК Валюта,
			|		ОборотыБюджетовДебет.КоличествоОборот * ДебетующиеСтатьи.КоэффициентДляКоличества КАК Количество,
			|		ОборотыБюджетовДебет.СуммаУпрОборот * ДебетующиеСтатьи.КоэффициентДляСуммы КАК СуммаУпр,
			|		ОборотыБюджетовДебет.ВалютнаяСуммаОборот * ДебетующиеСтатьи.КоэффициентДляСуммы КАК ВалютнаяСумма
			|	ИЗ
			|		РегистрНакопления.ОборотыБюджетов.Обороты(&ДатаНач_"+Номер+", &ДатаКон_"+Номер+","+ПериодДанных+",Сценарий = &Сценарий_"+Номер+") КАК ОборотыБюджетовДебет
			...
			|			
			|			ГДЕ
			|				СтатьиШаблоныПроводок.СчетДт В ИЕРАРХИИ(&СтатьяБаланса)) КАК ДебетующиеСтатьи
			|			ПО ОборотыБюджетовДебет.СтатьяОборотов = ДебетующиеСтатьи.Ссылка
			|	
			|	ОБЪЕДИНИТЬ ВСЕ
			|	
			|	ВЫБРАТЬ
			|		ОборотыБюджетовКредит.СтатьяОборотов,
			|		ОборотыБюджетовКредит.Период,
			|		ОборотыБюджетовКредит.Сценарий КАК Сценарий,
			|		ОборотыБюджетовКредит.Проект,
			|		ОборотыБюджетовКредит.ЦФО,
			|		ОборотыБюджетовКредит.Контрагент,
			|		ОборотыБюджетовКредит.Номенклатура,
			|		ОборотыБюджетовКредит.Валюта,
			|		ОборотыБюджетовКредит.КоличествоОборот * КредитующиеСтатьи.КоэффициентДляКоличества * -(1),
			|		ОборотыБюджетовКредит.СуммаУпрОборот * КредитующиеСтатьи.КоэффициентДляСуммы * -(1),
			|		ОборотыБюджетовКредит.ВалютнаяСуммаОборот * КредитующиеСтатьи.КоэффициентДляСуммы * -(1)
			|	ИЗ
			|		РегистрНакопления.ОборотыБюджетов.Обороты(&ДатаНач_"+Номер+", &ДатаКон_"+Номер+", "+ПериодДанных+",Сценарий = &Сценарий_"+Номер+") КАК ОборотыБюджетовКредит
			|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
			|				СтатьиШаблоныПроводок.Ссылка КАК Ссылка,
			|				СтатьиШаблоныПроводок.КоэффициентДляСуммы КАК КоэффициентДляСуммы,
			|				СтатьиШаблоныПроводок.КоэффициентДляКоличества КАК КоэффициентДляКоличества
			...
			|			
			|			ГДЕ
			|				СтатьиШаблоныПроводок.СчетКт В ИЕРАРХИИ(&СтатьяБаланса)) КАК КредитующиеСтатьи
			|			ПО ОборотыБюджетовКредит.СтатьяОборотов = КредитующиеСтатьи.Ссылка) КАК ОборотыДтКт
			|СГРУППИРОВАТЬ ПО
			|Сценарий,
			|Период,
			|СтатьяОборотов,
			...
			Для Каждого СтрокаПоказатели Из Показатели Цикл
				...
				Для Индекс=0 По ТаблицаСценарии.Количество()-1 Цикл
					...
					Если Номер = Индекс Тогда
						...
						ТекстЗапроса = ТекстЗапроса + ",
						|	СУММА("+СтрокаПоказатели.Имя+"Оборот) КАК " + СтрокаПоказатели.Имя+"_"+Индекс;
...
Процедура СформироватьНастройкиОтчета(МассивОтбора,СтруктураПредставлениеПолей)
	...
	ТекстЗапроса="
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|ДанныеСценариев.СтатьяОборотов КАК СтатьяОборотов
	|{ВЫБРАТЬ
	|ДанныеСценариев.СтатьяОборотов.* КАК СтатьяОборотов,
	...
	|{ГДЕ
	|ДанныеСценариев.СтатьяОборотов.*,
	|ДанныеСценариев.ЦФО.*,
	...
	|ПО
	|ОБЩИЕ,
	|ДанныеСценариев.СтатьяОборотов Иерархия
	|{ИТОГИ ПО
	|ДанныеСценариев.НомерСценария,
	|ДанныеСценариев.СтатьяОборотов.*,
	|ДанныеСценариев.Период,
...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	мНазваниеОтчета = ?(НЕ СтатьяБаланса.Пустая(),"Сравнительный анализ бюджетов движений: "+СтатьяБаланса.Наименование,"Сравнительный анализ оборотов");
	мНаДату = Ложь;
...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Ложь, ВысотаЗаголовка = 0, ТолькоЗаголовок = Ложь) Экспорт
	...
	Для Каждого Реквизит Из Метаданные.Отчеты.СравнительныйАнализОборотовПоСтатьямБюджетов.Реквизиты Цикл
...
Процедура ВывестиСтрокуШапки(ВыборкаГоризонтальная, СтруктураПараметров, ЗначениеВыборкиДанные,СтрокаДанныхОтчета)
	...
	ОбластьЗначениеГруппировкиГор.Параметры.РасшифровкаДанные=Новый Структура("Сценарий,ДатаНач,ДатаКон,Период",
...
Процедура СоздатьОперацию(Расшифровка, ЭтотОтчет)
	...
	Если Расшифровка.Свойство("Сценарий") и НЕ Расшифровка.Сценарий.ЭтоГруппа Тогда
...
Процедура РасшифровкаДвижений(Расшифровка,ЭтотОтчет,Выбор)
	...
	Для Инд=0 По ПостроительОтчета.ИзмеренияСтроки.Количество()-1 Цикл
		...
		Если ПостроительОтчета.ИзмеренияСтроки[Инд].Имя="НомерСценария" ИЛИ ПостроительОтчета.ИзмеренияСтроки[Инд].Имя="Период" Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ОбработкаРасшифровкиОтчетаБюджетирования(Расшифровка, СтандартнаяОбработка, ЭтотОтчет) Экспорт
	...
	Если ТипЗнч(Расшифровка) = Тип("Структура") Тогда
		...
		ИначеЕсли Выбор.Значение=0 ИЛИ Выбор.Значение=-1 Тогда
