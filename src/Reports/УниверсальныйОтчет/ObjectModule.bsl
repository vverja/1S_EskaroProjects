...
Процедура СформироватьОтчет(ТабличныйДокумент, Очищать = Истина, ФиксироватьЗаголовок = Истина, Отчет = Неопределено) Экспорт
	...
	Если ВыводВозможен() Тогда
		...
		Если РольДоступна("ГруппаПолный") ИЛИ РольДоступна("ГруппаА") Тогда
			СтараяПустаяДата = Дата(2009,1,1,0,0,0);
			ПостроительОтчета.Параметры.Вставить("НПТ", Константы.НПТ.Получить());
			Если ДатаНач < ПараметрыСеанса.КонтрольнаяДатаОграничения 
				И ДатаКон < ПараметрыСеанса.КонтрольнаяДатаОграничения Тогда
				
				//ДатаН = НачалоДня(ПараметрыСеанса.КонтрольнаяДатаОграничения);
				ПостроительОтчета.Параметры.Вставить("ДатаНач", СтараяПустаяДата);
				ПостроительОтчета.Параметры.Вставить("ДатаНачала", СтараяПустаяДата);
				ПостроительОтчета.Параметры.Вставить("ДатаКон", СтараяПустаяДата);
				ПостроительОтчета.Параметры.Вставить("ДатаКонца", СтараяПустаяДата);
			Иначе	
				Если ДатаНач < ПараметрыСеанса.КонтрольнаяДатаОграничения Тогда
					ДатаН = НачалоДня(ПараметрыСеанса.КонтрольнаяДатаОграничения);
					ДатаК = ДатаКон;
					ПостроительОтчета.Параметры.Вставить("ДатаНач", ?(ДатаН = Дата('00010101000000'), ДатаН, Новый Граница(ДатаН, ВидГраницы.Включая)));			
					ПостроительОтчета.Параметры.Вставить("ДатаНачала", ?(ДатаН = Дата('00010101000000'), ДатаН, НачалоДня(ДатаН)));
					Если ДатаН > ДатаКон Тогда
						ДатаК = ДатаН;		
					КонецЕсли;
					ПостроительОтчета.Параметры.Вставить("ДатаКон", ?(ДатаК = Дата('00010101000000'), ДатаК, Новый Граница(КонецДня(ДатаК), ВидГраницы.Включая)));
					ПостроительОтчета.Параметры.Вставить("ДатаКонца", ?(ДатаК = Дата('00010101000000'), Дата("39991231"), КонецДня(ДатаК)));
				Иначе
...
Процедура УстановитьНачальныеНастройкиПоМетаданнымРегистра(ИмяРегистра)
	...
	Если РольДоступна("ГруппаА") Тогда
		Если Найти(ОписаниеПолейВыборки, "Организация") <> 0 Тогда
			НПТ = Символы.ПС + " ГДЕ ИсточникДанных.Организация <> &НПТ ";	
		Иначе
			НПТ = Символы.ПС + " ГДЕ ИсточникДанных.Регистратор.Организация <> &НПТ ";
		КонецЕсли; 	
	КонецЕсли; 
	ОписаниеОтборовРезультатаЗапроса = НПТ + "
...
Процедура ДобавитьВТекстЗапросаДанныеОФизлице(ТекстЗапроса, ТаблицаПолей = Неопределено, СтруктураПредставлениеПолей = Неопределено, Параметры = Неопределено, СоответствиеДанныхОФизлицеИИзмерений = Неопределено, СтруктураЗаменять = Неопределено) Экспорт
	...
	Для каждого Поле из ТаблицаПолей Цикл
		...
		|	" + Поле.ПутьКДанным + ".КодПоДРФО КАК " + Поле.ИмяИзмерения + "КодПоДРФО,
		...
		|	ГражданствоФизЛиц.НеЯвляетсяНалоговымРезидентом КАК " + Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентом,
		...
		|	ВоинскийУчет.ОснованиеНегодности КАК " + Поле.ИмяИзмерения + "ОснованиеНегодности,
		|	ВоинскийУчет.СпецУчет КАК " + Поле.ИмяИзмерения + "СпецУчет,
		|	ВоинскийУчет.Группа КАК " + Поле.ИмяИзмерения + "Группа,
		|	ВоинскийУчет.Категория КАК " + Поле.ИмяИзмерения + "Категория,
		|	ВоинскийУчет.Профиль КАК " + Поле.ИмяИзмерения + "Профиль,
		...
		|	" + Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентом,
		...
		|	" + Поле.ИмяИзмерения + "ОснованиеНегодности,
		|	" + Поле.ИмяИзмерения + "СпецУчет,
        |	" + Поле.ИмяИзмерения + "Группа,		
		|	" + Поле.ИмяИзмерения + "Категория,
		|	" + Поле.ИмяИзмерения + "Профиль,
		...
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "КодПоДРФО),
		...
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентом),
		...
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "ОснованиеНегодности),
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "СпецУчет),
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "Группа),
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "Категория),
		|	МАКСИМУМ(" + Поле.ИмяИзмерения + "Профиль),
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "КодПоДРФО", "Код ДРФО");
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентом", "Не является налоговым резидентом");
		...
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "ОснованиеНегодности", "Основание негодности");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "СпецУчет", "Спец учет");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Группа", "Группа");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Категория", "Категория");
		СтруктураПредставлениеПолей.Вставить(Поле.ИмяИзмерения + "Профиль", "Профиль");
		...
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "КодПоДРФО", Поле.ИмяИзмерения);
		...
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "НеЯвляетсяНалоговымРезидентом", Поле.ИмяИзмерения);
		...
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "ОснованиеНегодности", Поле.ИмяИзмерения);
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "СпецУчет", Поле.ИмяИзмерения);	
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "Группа", Поле.ИмяИзмерения);
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "Категория", Поле.ИмяИзмерения);
		СоответствиеДанныхОФизлицеИИзмерений.Вставить(Поле.ИмяИзмерения + "Профиль", Поле.ИмяИзмерения);
