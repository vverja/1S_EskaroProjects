...
Процедура СформироватьОтчет(ДокументРезультат, ПоказыватьЗаголовок = Истина, ВысотаЗаголовка = 0) Экспорт
	...
	Пока ВыборкаСчетаКт.Следующий() Цикл
		...
		Если (НЕ ПоСубсчетам и СтрДлина(ВыборкаСчетаКт.СчетКтПредставление)>2) И СоотвСчетаВерхнегоУровня[ВыборкаСчетаКт.СчетКт]=Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	Пока ВыборкаСчетаДт.Следующий() Цикл
		...
		Если (НЕ ПоСубсчетам и СтрДлина(ВыборкаСчетаДт.СчетДтПредставление)>2) И СоотвСчетаВерхнегоУровня[ВыборкаСчетаДт.СчетДт]=Неопределено Тогда
			Продолжить;
		КонецЕсли;
	...
	//			КонецЕсли;	
	//		КонецЕсли;  
	//		СтруктураНастроекОтчета.Вставить("ДатаНач", ДатаНачМоя);
	//		СтруктураНастроекОтчета.Вставить("ДатаКон", ДатаКонМоя);
	//	Иначе
...
Процедура ВывестиОборот(ВыборкаСчетаДт, СоотвСчетаВерхнегоУровня, СчетДтПоВалюте, СоответствиеСчетовКтИВалют, ДокументРезультат, 
	...
	Пока ВыборкаОборот.Следующий() Цикл
		...
		Если (НЕ ПоСубсчетам и (СтрДлина(ВыборкаОборот.СчетКтПредставление)>2 или СтрДлина(ВыборкаОборот.СчетДтПредставление)>2 )) И СоотвСчетаВерхнегоУровня[ВыборкаОборот.СчетКт]=Неопределено Тогда
			Продолжить;
		КонецЕсли;
